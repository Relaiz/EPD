using Microsoft.EntityFrameworkCore;
using ReactiveUI;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Security.Cryptography;
using System.Text;
using System.Threading;
using System.Threading.Tasks;
using TeacherScheduleApp.Data;
using TeacherScheduleApp.Helpers;
using TeacherScheduleApp.Messages;
using TeacherScheduleApp.Models;
using static TeacherScheduleApp.Services.EventService;

namespace TeacherScheduleApp.Services
{
    public class EventService
    {
        public const int DefaultEmployeeId = 1;
        private const int QUANTUM_MIN = 5;
        private const int MIN_BALANCE_EDGE_BLOCK_MIN = QUANTUM_MIN * 2;

        private bool _isBalancingNow;
        private static readonly AsyncLocal<bool> _suppressReactiveMessages = new();

        public static bool IsWorkLike(Event e)
            => e.EventType.IsAutoAdjustableWork();

        public static bool IsTeaching(Event e)
            => e.EventType.IsTeaching();

        public static bool IsCreditedWorkTime(Event e)
            => e.EventType.IsCreditedWorkTime() || e.EventType == EventType.BusinessTrip;

        private static bool IsBalanceAdjustableWork(Event e)
            => e.IsAutoGenerated && e.EventType == EventType.Work;

        private bool HasBalanceAdjustableWork(DateTime day, int employeeId = DefaultEmployeeId)
            => GetEventsForDay(employeeId, day)
                .Any(e => !e.IsDeleted && IsBalanceAdjustableWork(e));

        private static int ResolveEmployeeId(Event ev)
            => ev.EmployeeId > 0 ? ev.EmployeeId : DefaultEmployeeId;

        private static int RoundDownToQuantum(int minutes) => minutes - minutes % QUANTUM_MIN;
        private static int RoundUpToQuantum(int minutes) => minutes % QUANTUM_MIN == 0 ? minutes : minutes + (QUANTUM_MIN - minutes % QUANTUM_MIN);
        private static TimeSpan QMinutes(int minutes) => TimeSpan.FromMinutes(RoundDownToQuantum(minutes));
        private static int ToWholeMinutes(double hours) => (int)Math.Round(hours * 60.0);

        private static DateTime TruncToMinute(DateTime dt)
            => new DateTime(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, 0, dt.Kind);

        public static bool IsWorkday(DateTime d)
            => d.DayOfWeek is not DayOfWeek.Saturday and not DayOfWeek.Sunday
               && !HolidayHelper.IsCzechHoliday(d);

        private static DateTime Min(DateTime a, DateTime b) => a < b ? a : b;
        private static DateTime Max(DateTime a, DateTime b) => a > b ? a : b;

        private static List<DateTime> GetWeekWorkdays(DateTime anyDate)
        {
            var date = anyDate.Date;
            int delta = ((int)date.DayOfWeek - (int)DayOfWeek.Monday + 7) % 7;
            var weekStart = date.AddDays(-delta);

            return Enumerable.Range(0, 7)
                .Select(i => weekStart.AddDays(i))
                .Where(IsWorkday)
                .ToList();
        }

        private sealed class ReactiveMessageSuppressionScope : IDisposable
        {
            private readonly bool _prev;

            public ReactiveMessageSuppressionScope()
            {
                _prev = _suppressReactiveMessages.Value;
                _suppressReactiveMessages.Value = true;
            }

            public void Dispose()
            {
                _suppressReactiveMessages.Value = _prev;
            }
        }

        public IDisposable BeginNotificationSuppression()
            => new ReactiveMessageSuppressionScope();

        private static bool AreReactiveMessagesSuppressed
            => _suppressReactiveMessages.Value;

        private static void NotifyAutoEventsGenerated()
        {
            if (!AreReactiveMessagesSuppressed)
                MessageBus.Current.SendMessage(new AutoEventsGeneratedMessage());
        }

        private static void NotifyUserSettingsChanged(DateTime day)
        {
            if (!AreReactiveMessagesSuppressed)
                MessageBus.Current.SendMessage(new UserSettingsChangedMessage(day.Date));
        }

        public bool HasAnyAutoGeneratedEventsInYear(int year, int employeeId = DefaultEmployeeId)
        {
            var start = new DateTime(year, 1, 1);
            var end = start.AddYears(1);

            using var db = new AppDbContext();

            return db.Events
                .AsNoTracking()
                .Any(e =>
                    e.EmployeeId == employeeId &&
                    !e.IsDeleted &&
                    e.IsAutoGenerated &&
                    (
                        (e.AutoGeneratedForDate.HasValue &&
                         e.AutoGeneratedForDate.Value >= start &&
                         e.AutoGeneratedForDate.Value < end)
                        ||
                        (e.StartTime >= start && e.StartTime < end)
                        ||
                        (e.EndTime >= start && e.EndTime < end)
                    ));
        }

        public bool IsYearInitialized(int year, int employeeId = DefaultEmployeeId)
            => GetUninitializedScopesForYear(year, employeeId).Count == 0;

        public List<ChangedRange> GetUninitializedScopesForYear(int year, int employeeId = DefaultEmployeeId)
        {
            var firstDay = new DateTime(year, 1, 1);
            var lastDay = new DateTime(year, 12, 31);

            var workdays = Enumerable.Range(0, (lastDay - firstDay).Days + 1)
                .Select(i => firstDay.AddDays(i).Date)
                .Where(IsWorkday)
                .ToList();

            if (workdays.Count == 0)
                return new List<ChangedRange>();

            using var cache = BeginEventReadCache(employeeId, workdays);

            var uninitializedScopes = workdays
                .GroupBy(d => (
                    IsoYear: System.Globalization.ISOWeek.GetYear(d),
                    IsoWeek: System.Globalization.ISOWeek.GetWeekOfYear(d)))
                .OrderBy(g => g.Key.IsoYear)
                .ThenBy(g => g.Key.IsoWeek)
                .Select(g =>
                {
                    var weekDays = g
                        .OrderBy(d => d)
                        .ToList();

                    var anyDay = weekDays[0];
                    int isoYear = g.Key.IsoYear;
                    int isoWeek = g.Key.IsoWeek;
                    var fingerprint = Helpers.BalanceFingerprint.ForWeek(this, employeeId, anyDay);

                    bool isBalanced = Helpers.WeekBalanceStore.IsBalanced(employeeId, isoYear, isoWeek, fingerprint);

                    return new
                    {
                        IsBalanced = isBalanced,
                        Scope = new ChangedRange(weekDays[0], weekDays[^1])
                    };
                })
                .Where(x => !x.IsBalanced)
                .Select(x => x.Scope)
                .ToList();

            return MergeChangedRanges(uninitializedScopes);
        }

        private static List<ChangedRange> MergeChangedRanges(IEnumerable<ChangedRange> scopes)
        {
            var list = scopes
                .Where(x => x.HasValue)
                .Select(x => x.Normalize())
                .OrderBy(x => x.Start)
                .ToList();

            if (list.Count == 0)
                return new List<ChangedRange>();

            var merged = new List<ChangedRange> { list[0] };

            for (int i = 1; i < list.Count; i++)
            {
                var last = merged[^1];
                var cur = list[i];

                if (cur.Start!.Value <= last.End!.Value.AddDays(1))
                {
                    merged[^1] = new ChangedRange(
                        last.Start,
                        cur.End!.Value > last.End!.Value ? cur.End : last.End);
                }
                else
                {
                    merged.Add(cur);
                }
            }

            return merged;
        }

        private static List<(DateTime s, DateTime e)> MergeIntervals(IEnumerable<(DateTime s, DateTime e)> intervals)
        {
            var sorted = intervals
                .Where(x => x.e > x.s)
                .OrderBy(x => x.s)
                .ToList();

            var merged = new List<(DateTime s, DateTime e)>();

            foreach (var seg in sorted)
            {
                if (merged.Count == 0 || merged[^1].e < seg.s)
                    merged.Add(seg);
                else
                    merged[^1] = (merged[^1].s, merged[^1].e > seg.e ? merged[^1].e : seg.e);
            }

            return merged;
        }

        private static List<(DateTime s, DateTime e, List<Event> owners)> BuildMergedWithOwners(IEnumerable<Event> evs, DateTime winS, DateTime winE)
        {
            var segs = evs
                .Select(e => (
                    s: e.StartTime < winS ? winS : e.StartTime,
                    e: e.EndTime > winE ? winE : e.EndTime,
                    ev: e))
                .Where(x => x.e > x.s)
                .OrderBy(x => x.s)
                .ToList();

            var merged = new List<(DateTime s, DateTime e, List<Event> owners)>();

            foreach (var seg in segs)
            {
                if (merged.Count == 0 || merged[^1].e < seg.s)
                {
                    merged.Add((seg.s, seg.e, new List<Event> { seg.ev }));
                }
                else
                {
                    if (seg.e > merged[^1].e)
                        merged[^1] = (merged[^1].s, seg.e, merged[^1].owners);

                    merged[^1].owners.Add(seg.ev);
                }
            }

            return merged;
        }

        private static DateTime FirstDayOfIsoWeek(int isoYear, int isoWeek)
        {
            var thursday = new DateTime(isoYear, 1, 4);
            int delta = System.Globalization.ISOWeek.GetWeekOfYear(thursday) == 1 ? 0 : 7;
            var monday = thursday.AddDays(-((int)thursday.DayOfWeek - (int)DayOfWeek.Monday + 7) % 7);
            return monday.AddDays(7 * (isoWeek - 1) - delta);
        }

        private static ResolvedDaySettings GetResolvedWindow(DateTime day, int employeeId)
            => SettingsService.GetResolvedDaySettings(day.Date, employeeId);

        private static ResolvedDaySettings GetBaseResolvedWindow(DateTime day, int employeeId)
            => SettingsService.GetResolvedDaySettingsIgnoringComputed(day.Date, employeeId);

        private void InvalidateMonthsForDates(IEnumerable<DateTime> dates)
        {
            var months = dates.Select(d => new { d.Year, d.Month }).Distinct().ToList();
            foreach (var m in months)
                MonthBalanceStore.Invalidate(m.Year, m.Month);
        }

        public List<Event> GetAllEvents(int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();
            return db.Events
                .AsNoTracking()
                .Where(x => x.EmployeeId == employeeId)
                .ToList();
        }

        public List<Event> LoadEvents(int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();
            return db.Events
                .AsNoTracking()
                .Where(e => e.EmployeeId == employeeId && !e.IsDeleted)
                .ToList();
        }

        public Event? GetEventById(int id, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();
            return db.Events
                .AsNoTracking()
                .FirstOrDefault(e => e.Id == id && e.EmployeeId == employeeId);
        }

        public Event? FindEventByStartTime(DateTime startTime, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();
            return db.Events
                .AsNoTracking()
                .FirstOrDefault(e =>
                    e.EmployeeId == employeeId &&
                    e.StartTime == startTime &&
                    !e.IsDeleted);
        }

        public List<Event> GetEventsForDay(int employeeId, DateTime date)
        {
            var day = date.Date;
            var scope = GetEventReadCacheScopeForDay(employeeId, day);

            if (scope != null)
            {
                if (!scope.TryGet(day, out var cached))
                {
                    cached = LoadEventsForDayFromDatabase(employeeId, day);
                    scope.Set(day, cached);
                }

                return cached.ToList();
            }

            return LoadEventsForDayFromDatabase(employeeId, day);
        }

        public List<Event> GetEventsForWeek(int employeeId, DateTime anyDate)
        {
            var date = anyDate.Date;
            int delta = (int)date.DayOfWeek - (int)DayOfWeek.Monday;
            if (delta < 0) delta += 7;

            var weekStart = date.AddDays(-delta);
            var weekEnd = weekStart.AddDays(7);

            using var db = new AppDbContext();

            return db.Events
                .AsNoTracking()
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime < weekEnd &&
                            e.EndTime >= weekStart)
                .OrderBy(e => e.StartTime)
                .ToList();
        }

        public List<Event> GetEventsForMonth(int employeeId, DateTime date)
        {
            var monthStart = new DateTime(date.Year, date.Month, 1);
            var monthEnd = monthStart.AddMonths(1);

            using var db = new AppDbContext();

            return db.Events
                .AsNoTracking()
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime >= monthStart &&
                            e.StartTime < monthEnd)
                .OrderBy(e => e.StartTime)
                .ToList();
        }

        public List<Event> GetEventsForRange(int employeeId, DateTime start, DateTime end)
        {
            using var db = new AppDbContext();

            return db.Events
                .AsNoTracking()
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime < end &&
                            e.EndTime >= start)
                .OrderBy(e => e.StartTime)
                .ToList();
        }

        public void CreateAutoEventsBulk(IEnumerable<Event> events)
        {
            var list = events?
                .Where(e => e != null)
                .ToList() ?? new List<Event>();

            if (list.Count == 0)
                return;

            using var db = new AppDbContext();
            db.ChangeTracker.AutoDetectChangesEnabled = false;

            foreach (var ev in list)
            {
                if (ev.EmployeeId <= 0)
                    ev.EmployeeId = DefaultEmployeeId;
            }

            db.Events.AddRange(list);
            db.SaveChanges();

            var byEmployee = list
                .GroupBy(x => x.EmployeeId)
                .ToList();

            foreach (var grp in byEmployee)
                InvalidateEventCacheDays(grp.Key, CollectAffectedDates(grp));
        }

        public void CreateEventsBulk(IEnumerable<Event> events)
        {
            var list = events?
                .Where(e => e != null)
                .ToList() ?? new List<Event>();

            if (list.Count == 0)
                return;

            using var db = new AppDbContext();
            db.ChangeTracker.AutoDetectChangesEnabled = false;

            foreach (var ev in list)
            {
                if (ev.EmployeeId <= 0)
                    ev.EmployeeId = DefaultEmployeeId;
            }

            db.Events.AddRange(list);
            db.SaveChanges();

            var byEmployee = list
                .GroupBy(x => x.EmployeeId)
                .ToList();

            foreach (var grp in byEmployee)
                InvalidateEventCacheDays(grp.Key, CollectAffectedDates(grp));
        }

        public void CreateAutoEvent(Event ev)
        {
            using var db = new AppDbContext();

            if (ev.EmployeeId <= 0)
                ev.EmployeeId = DefaultEmployeeId;

            db.Events.Add(ev);
            db.SaveChanges();

            InvalidateEventCacheDays(ev.EmployeeId, EnumerateDatesInclusive(ev.StartTime, ev.EndTime));
        }

        public ChangedRange CreateEventRaw(Event ev)
        {
            var employeeId = ResolveEmployeeId(ev);
            var affectedDates = new List<DateTime>();

            if (ev.IsAutoGenerated)
            {
                using var dbContext = new AppDbContext();
                ev.EmployeeId = employeeId;
                dbContext.Events.Add(ev);
                dbContext.SaveChanges();

                affectedDates = EnumerateDatesInclusive(ev.StartTime, ev.EndTime);

                InvalidateEventCacheDays(employeeId, affectedDates);
                InvalidateMonthsForDates(affectedDates);
                InvalidateWeeksForDates(affectedDates, employeeId);

                return ChangedRange.FromDates(affectedDates);
            }

            DateTime parentDate = ev.StartTime.Date;

            while (parentDate <= ev.EndTime.Date &&
                   (parentDate.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday ||
                    HolidayHelper.IsCzechHoliday(parentDate)))
            {
                parentDate = parentDate.AddDays(1);
            }

            if (parentDate > ev.EndTime.Date)
                return ChangedRange.Empty;

            using var db = new AppDbContext();

            var parent = new Event
            {
                EmployeeId = employeeId,
                Title = ev.Title,
                Description = ev.Description,
                EventType = ev.EventType,
                AllDay = ev.AllDay,
                StartTime = parentDate + ev.StartTime.TimeOfDay,
                EndTime = parentDate + ev.EndTime.TimeOfDay,
                ParentEventId = null,
                IsAutoGenerated = false,
                ImportBatchId = ev.ImportBatchId,
                IsDeleted = false,
                HasCollision = ev.HasCollision,
                AutoGeneratedForDate = null
            };

            db.Events.Add(parent);
            db.SaveChanges();

            affectedDates = new List<DateTime> { parentDate };
            var createdEvents = new List<Event> { parent };

            for (var day = parentDate.AddDays(1); day <= ev.EndTime.Date; day = day.AddDays(1))
            {
                if (day.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday || HolidayHelper.IsCzechHoliday(day))
                    continue;

                var child = new Event
                {
                    EmployeeId = employeeId,
                    Title = ev.Title,
                    Description = ev.Description,
                    EventType = ev.EventType,
                    AllDay = ev.AllDay,
                    StartTime = day + ev.StartTime.TimeOfDay,
                    EndTime = day + ev.EndTime.TimeOfDay,
                    ParentEventId = parent.Id,
                    IsAutoGenerated = false,
                    ImportBatchId = ev.ImportBatchId,
                    IsDeleted = false,
                    HasCollision = ev.HasCollision,
                    AutoGeneratedForDate = null
                };

                db.Events.Add(child);
                createdEvents.Add(child);
                affectedDates.Add(day);
            }

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDates);
            InvalidateMonthsForDates(affectedDates);
            InvalidateWeeksForDates(affectedDates, employeeId);

            var changedRange = ChangedRange.FromDates(affectedDates);
            var priorityRange = ResolveNewWorkOverOldWorkOverlaps(
                createdEvents.Select(x => x.Id),
                employeeId);

            return changedRange.Merge(priorityRange);
        }

        public void CreateEvent(Event ev)
        {
            CreateEventRaw(ev);
        }

        public ChangedRange UpdateEventRaw(Event ev)
        {
            using var db = new AppDbContext();

            var employeeId = ResolveEmployeeId(ev);

            var parent = db.Events.SingleOrDefault(x => x.Id == ev.Id && x.EmployeeId == employeeId);
            if (parent == null)
                return ChangedRange.Empty;

            var oldStart = parent.StartTime.Date;
            var oldEnd = parent.EndTime.Date;

            var wasAutoGenerated = parent.IsAutoGenerated;

            parent.Title = ev.Title;
            parent.StartTime = ev.StartTime;
            parent.EndTime = ev.EndTime;
            parent.AllDay = ev.AllDay;
            parent.Description = ev.Description;
            parent.IsDeleted = ev.IsDeleted;
            parent.EventType = ev.EventType;
            parent.HasCollision = ev.HasCollision;
            parent.ImportBatchId = ev.ImportBatchId;

            if (wasAutoGenerated)
            {
                parent.IsAutoGenerated = false;
                parent.AutoGeneratedForDate = null;
                parent.ParentEventId = null;
            }
            else
            {
                parent.IsAutoGenerated = ev.IsAutoGenerated;
                parent.AutoGeneratedForDate = ev.AutoGeneratedForDate;
            }

            var oldChildren = db.Events
                .Where(c => c.EmployeeId == employeeId && c.ParentEventId == parent.Id)
                .ToList();

            if (oldChildren.Count > 0)
                db.Events.RemoveRange(oldChildren);

            DateTime start = parent.StartTime.Date.AddDays(1);
            var savedEvents = new List<Event> { parent };

            for (var day = start; day <= ev.EndTime.Date; day = day.AddDays(1))
            {
                if (day.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday || HolidayHelper.IsCzechHoliday(day))
                    continue;

                var child = new Event
                {
                    EmployeeId = employeeId,
                    Title = parent.Title,
                    Description = parent.Description,
                    EventType = parent.EventType,
                    AllDay = parent.AllDay,
                    StartTime = day + parent.StartTime.TimeOfDay,
                    EndTime = day + parent.EndTime.TimeOfDay,
                    ParentEventId = parent.Id,
                    IsAutoGenerated = false,
                    ImportBatchId = parent.ImportBatchId,
                    IsDeleted = false,
                    HasCollision = parent.HasCollision,
                    AutoGeneratedForDate = null
                };

                db.Events.Add(child);
                savedEvents.Add(child);
            }

            db.SaveChanges();

            var affectedDates = new HashSet<DateTime>(
                EnumerateDatesInclusive(oldStart, oldEnd)
                    .Concat(EnumerateDatesInclusive(parent.StartTime.Date, parent.EndTime.Date)));

            InvalidateEventCacheDays(employeeId, affectedDates);
            InvalidateMonthsForDates(affectedDates);
            InvalidateWeeksForDates(affectedDates, employeeId);

            var changedRange = ChangedRange.FromDates(affectedDates);
            var priorityRange = ResolveNewWorkOverOldWorkOverlaps(
                savedEvents.Select(x => x.Id),
                employeeId);

            return changedRange.Merge(priorityRange);
        }

        public void UpdateEvent(Event ev, bool suppressRegen = false)
        {
            UpdateEventRaw(ev);
        }

        private ChangedRange ResolveNewWorkOverOldWorkOverlaps(IEnumerable<int> winnerIds, int employeeId)
        {
            var winnersSet = winnerIds
                .Where(id => id > 0)
                .Distinct()
                .ToHashSet();

            if (winnersSet.Count == 0)
                return ChangedRange.Empty;

            using var db = new AppDbContext();

            var winners = db.Events
                .Where(e =>
                    e.EmployeeId == employeeId &&
                    winnersSet.Contains(e.Id) &&
                    !e.IsDeleted &&
                    !e.IsAutoGenerated &&
                    e.EventType == EventType.Work)
                .OrderBy(e => e.StartTime)
                .ToList();

            if (winners.Count == 0)
                return ChangedRange.Empty;

            var affectedDates = new HashSet<DateTime>();
            bool changed = false;

            foreach (var winner in winners)
            {
                var victims = db.Events
                    .Where(e =>
                        e.EmployeeId == employeeId &&
                        !e.IsDeleted &&
                        e.EventType == EventType.Work &&
                        !winnersSet.Contains(e.Id) &&
                        e.StartTime < winner.EndTime &&
                        e.EndTime > winner.StartTime)
                    .OrderBy(e => e.StartTime)
                    .ToList();

                foreach (var victim in victims)
                {
                    var originalStart = victim.StartTime;
                    var originalEnd = victim.EndTime;

                    var remains = SubtractOneInterval(
                        victim.StartTime,
                        victim.EndTime,
                        winner.StartTime,
                        winner.EndTime);

                    if (remains.Count == 1 &&
                        remains[0].s == victim.StartTime &&
                        remains[0].e == victim.EndTime)
                    {
                        continue;
                    }

                    foreach (var d in EnumerateDatesInclusive(originalStart, originalEnd))
                        affectedDates.Add(d);

                    if (remains.Count == 0)
                    {
                        victim.IsDeleted = true;
                        changed = true;
                        continue;
                    }

                    var first = remains[0];
                    victim.AllDay = false;
                    victim.StartTime = first.s;
                    victim.EndTime = first.e;
                    changed = true;

                    foreach (var d in EnumerateDatesInclusive(first.s, first.e))
                        affectedDates.Add(d);

                    foreach (var segment in remains.Skip(1))
                    {
                        db.Events.Add(new Event
                        {
                            EmployeeId = victim.EmployeeId,
                            Title = victim.Title,
                            Description = victim.Description,
                            EventType = victim.EventType,
                            AllDay = false,
                            StartTime = segment.s,
                            EndTime = segment.e,
                            ParentEventId = victim.ParentEventId,
                            IsAutoGenerated = victim.IsAutoGenerated,
                            ImportBatchId = victim.ImportBatchId,
                            IsDeleted = false,
                            HasCollision = victim.HasCollision,
                            AutoGeneratedForDate = victim.AutoGeneratedForDate
                        });

                        foreach (var d in EnumerateDatesInclusive(segment.s, segment.e))
                            affectedDates.Add(d);
                    }
                }
            }

            if (!changed)
                return ChangedRange.Empty;

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDates);
            InvalidateMonthsForDates(affectedDates);
            InvalidateWeeksForDates(affectedDates, employeeId);

            return ChangedRange.FromDates(affectedDates);
        }

        private static List<(DateTime s, DateTime e)> SubtractOneInterval(
            DateTime sourceStart,
            DateTime sourceEnd,
            DateTime blockerStart,
            DateTime blockerEnd)
        {
            var result = new List<(DateTime s, DateTime e)>();

            if (sourceEnd <= sourceStart)
                return result;

            if (blockerEnd <= sourceStart || blockerStart >= sourceEnd)
            {
                result.Add((sourceStart, sourceEnd));
                return result;
            }

            if (blockerStart > sourceStart)
                result.Add((sourceStart, blockerStart < sourceEnd ? blockerStart : sourceEnd));

            if (blockerEnd < sourceEnd)
                result.Add((blockerEnd > sourceStart ? blockerEnd : sourceStart, sourceEnd));

            return result
                .Where(x => x.e > x.s)
                .ToList();
        }

        public ChangedRange DeleteEventRaw(int id, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();

            var ev = db.Events.FirstOrDefault(e => e.Id == id && e.EmployeeId == employeeId && !e.IsDeleted);
            if (ev == null)
                return ChangedRange.Empty;

            var affectedDates = EnumerateDatesInclusive(ev.StartTime, ev.EndTime);

            ev.IsDeleted = true;
            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDates);
            InvalidateMonthsForDates(affectedDates);
            InvalidateWeeksForDates(affectedDates, employeeId);

            return ChangedRange.FromDates(affectedDates);
        }

        public void DeleteEvent(int id, int employeeId = DefaultEmployeeId)
        {
            DeleteEventRaw(id, employeeId);
        }

        public ChangedRange DeleteEventCascadeRaw(int id, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();

            var toDelete = db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            (e.Id == id || e.ParentEventId == id))
                .ToList();

            if (toDelete.Count == 0)
                return ChangedRange.Empty;

            var affectedDates = CollectAffectedDates(toDelete);

            foreach (var e in toDelete)
                e.IsDeleted = true;

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDates);
            InvalidateMonthsForDates(affectedDates);
            InvalidateWeeksForDates(affectedDates, employeeId);

            return ChangedRange.FromDates(affectedDates);
        }

        public void DeleteEventCascadeAndCleanup(int id, int employeeId = DefaultEmployeeId)
        {
            DeleteEventCascadeRaw(id, employeeId);
        }

        public async Task<ChangedRange> BulkSoftDeleteImportedInRangeRawAsync(
      DateTime from,
      DateTime to,
      int employeeId = DefaultEmployeeId)
        {
            var start = from.Date;
            var endExclusive = to.Date.AddDays(1);

            await using var db = new AppDbContext();

            var affectedSpans = await db.Events
                .Where(x => x.EmployeeId == employeeId
                            && !x.IsDeleted
                            && x.ImportBatchId != null
                            && x.StartTime >= start
                            && x.StartTime < endExclusive)
                .Select(x => new { x.StartTime, x.EndTime })
                .ToListAsync();

            var affectedDays = affectedSpans
                .SelectMany(x => EnumerateDatesInclusive(x.StartTime, x.EndTime))
                .Distinct()
                .OrderBy(x => x)
                .ToList();

            if (affectedDays.Count == 0)
                return ChangedRange.Empty;

#if NET7_0_OR_GREATER
            await db.Events
                .Where(x => x.EmployeeId == employeeId
                            && !x.IsDeleted
                            && x.ImportBatchId != null
                            && x.StartTime >= start
                            && x.StartTime < endExclusive)
                .ExecuteUpdateAsync(u => u.SetProperty(x => x.IsDeleted, true));
#else
    var list = await db.Events
        .Where(x => x.EmployeeId == employeeId
                    && !x.IsDeleted
                    && x.ImportBatchId != null
                    && x.StartTime >= start
                    && x.StartTime < endExclusive)
        .ToListAsync();

    foreach (var ev in list)
        ev.IsDeleted = true;

    await db.SaveChangesAsync();
#endif

            InvalidateEventCacheDays(employeeId, affectedDays);
            InvalidateMonthsForDates(affectedDays);
            InvalidateWeeksForDates(affectedDays, employeeId);

            return ChangedRange.FromDates(affectedDays);
        }

        public int SoftDeleteImportedInRange(DateTime startIncl, DateTime endIncl, int employeeId = DefaultEmployeeId)
        {
            var start = startIncl.Date;
            var endExclusive = endIncl.Date.AddDays(1);

            using var db = new AppDbContext();
            db.ChangeTracker.AutoDetectChangesEnabled = false;

            var toDel = db.Events
                .Where(e => e.EmployeeId == employeeId
                         && !e.IsDeleted
                         && e.ImportBatchId != null
                         && e.StartTime >= start
                         && e.StartTime < endExclusive)
                .ToList();

            if (toDel.Count == 0)
                return 0;

            var affectedDays = CollectAffectedDates(toDel);

            foreach (var e in toDel)
                e.IsDeleted = true;

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDays);
            InvalidateMonthsForDates(affectedDays);
            InvalidateWeeksForDates(affectedDays, employeeId);

            return toDel.Count;
        }

        public async Task<ChangedRange> DeleteEventsByImportIdFastRawAsync(
    string batchId,
    int employeeId = DefaultEmployeeId)
        {
            await using var db = new AppDbContext();

            var affectedSpans = await db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.ImportBatchId == batchId)
                .Select(e => new { e.StartTime, e.EndTime })
                .ToListAsync();

            var affectedDays = affectedSpans
                .SelectMany(x => EnumerateDatesInclusive(x.StartTime, x.EndTime))
                .Distinct()
                .OrderBy(x => x)
                .ToList();

            if (affectedDays.Count == 0)
                return ChangedRange.Empty;

#if NET7_0_OR_GREATER
            await db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.ImportBatchId == batchId)
                .ExecuteUpdateAsync(s => s.SetProperty(x => x.IsDeleted, true));
#else
    var toUpdate = await db.Events
        .Where(e => e.EmployeeId == employeeId &&
                    !e.IsDeleted &&
                    e.ImportBatchId == batchId)
        .ToListAsync();

    foreach (var ev in toUpdate)
        ev.IsDeleted = true;

    await db.SaveChangesAsync();
#endif

            InvalidateEventCacheDays(employeeId, affectedDays);
            InvalidateMonthsForDates(affectedDays);
            InvalidateWeeksForDates(affectedDays, employeeId);

            return ChangedRange.FromDates(affectedDays);
        }

        public async Task BulkSoftDeleteImportedInRangeAsync(DateTime from, DateTime to, int employeeId = DefaultEmployeeId)
        {
            await BulkSoftDeleteImportedInRangeRawAsync(from, to, employeeId);
        }

        public void DeleteEventsByImportId(string batchId, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();

            var toDelete = db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.ImportBatchId == batchId)
                .ToList();

            if (toDelete.Count == 0)
                return;

            var affectedDates = CollectAffectedDates(toDelete);

            foreach (var ev in toDelete)
                ev.IsDeleted = true;

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDates);
            InvalidateMonthsForDates(affectedDates);
            InvalidateWeeksForDates(affectedDates, employeeId);
        }

        public async Task DeleteEventsByImportIdFastAsync(string batchId, int employeeId = DefaultEmployeeId)
        {
            await DeleteEventsByImportIdFastRawAsync(batchId, employeeId);
        }

        public void RemoveAutoGeneratedEvents(int employeeId, DateTime date)
        {
            var day = date.Date;

            using var db = new AppDbContext();

#if NET7_0_OR_GREATER
            db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            e.IsAutoGenerated &&
                            e.AutoGeneratedForDate == day)
                .ExecuteDelete();
#else
    var evs = db.Events
        .Where(e => e.EmployeeId == employeeId &&
                    e.IsAutoGenerated &&
                    e.AutoGeneratedForDate == day)
        .ToList();

    if (evs.Count == 0)
        return;

    db.Events.RemoveRange(evs);
    db.SaveChanges();
#endif

            InvalidateEventCacheDays(employeeId, new[] { day });
        }

        public static int PurgeSoftDeleted()
        {
            using var db = new AppDbContext();
            using var tx = db.Database.BeginTransaction();

            const int batchSize = 500;
            int totalDeleted = 0;

            while (true)
            {
                var leafIds = db.Events
                    .Where(e => e.IsDeleted)
                    .Where(e => !db.Events.Any(ch => ch.ParentEventId == e.Id))
                    .Select(e => e.Id)
                    .Take(batchSize)
                    .ToList();

                if (leafIds.Count == 0)
                    break;

                var batch = db.Events
                    .Where(e => leafIds.Contains(e.Id))
                    .ToList();

                db.Events.RemoveRange(batch);
                db.SaveChanges();

                totalDeleted += batch.Count;
            }

            var blocked = db.Events
                .Where(e => e.IsDeleted)
                .Select(e => new
                {
                    e.Id,
                    e.ParentEventId
                })
                .ToList();

            tx.Commit();
            return totalDeleted;
        }

        public void UpdateSingleEventEnd(int eventId, DateTime newEnd, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();

            var ev = db.Events.SingleOrDefault(e => e.Id == eventId &&
                                                    e.EmployeeId == employeeId &&
                                                    !e.IsDeleted);
            if (ev == null)
                return;

            var oldDates = EnumerateDatesInclusive(ev.StartTime, ev.EndTime);

            if (newEnd < ev.StartTime)
                newEnd = ev.StartTime;

            ev.EndTime = newEnd;
            db.SaveChanges();

            var affectedDates = oldDates
                .Concat(EnumerateDatesInclusive(ev.StartTime, ev.EndTime))
                .Distinct()
                .OrderBy(x => x)
                .ToList();

            InvalidateEventCacheDays(employeeId, affectedDates);
        }

        private (Event? first, Event? last) GetEdgeWorkEvents(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var evs = GetEventsForDay(employeeId, day)
                .Where(e => IsWorkLike(e) && !e.IsDeleted)
                .OrderBy(e => e.StartTime)
                .ToList();

            return (evs.FirstOrDefault(), evs.LastOrDefault());
        }

        private bool CanTrimStart(DateTime day, int employeeId = DefaultEmployeeId)
             => GetTrimStartCapacityMinutes(day, employeeId) >= QUANTUM_MIN;

        private bool CanTrimEnd(DateTime day, int employeeId = DefaultEmployeeId)
            => GetTrimEndCapacityMinutes(day, employeeId) >= QUANTUM_MIN;

        private bool IsLockedEdgeDay(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var evs = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType != EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .ThenBy(e => e.EndTime)
                .ToList();

            if (evs.Count == 0)
                return false;

            bool firstPinned = !IsBalanceAdjustableWork(evs.First());
            bool lastPinned = !IsBalanceAdjustableWork(evs.Last());

            return firstPinned && lastPinned;
        }

        private void DeleteLunchEvents(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            using var db = new AppDbContext();

            var lunches = db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime >= dayStart &&
                            e.StartTime < dayEnd &&
                            e.EventType == EventType.Lunch &&
                            e.IsAutoGenerated)
                .ToList();

            if (lunches.Count == 0)
                return;

            db.Events.RemoveRange(lunches);
            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, new[] { dayStart });
        }

        private void ReplaceLunchEvent(DateTime day, TimeSpan ls, TimeSpan le, int employeeId = DefaultEmployeeId)
        {
            DeleteLunchEvents(day, employeeId);

            if (le <= ls)
                return;

            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            using var db = new AppDbContext();

            bool hasAnyLunch = db.Events.Any(e =>
                e.EmployeeId == employeeId &&
                !e.IsDeleted &&
                e.StartTime >= dayStart &&
                e.StartTime < dayEnd &&
                e.EventType == EventType.Lunch);

            if (hasAnyLunch)
                return;

            db.Events.Add(new Event
            {
                EmployeeId = employeeId,
                Title = "Oběd (auto)",
                EventType = EventType.Lunch,
                StartTime = day + ls,
                EndTime = day + le,
                IsAutoGenerated = true,
                AutoGeneratedForDate = day.Date
            });

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, new[] { dayStart });
        }

        private void SplitAdjustableWorkAroundOneLunch(DateTime day, TimeSpan ls, TimeSpan le, int employeeId = DefaultEmployeeId)
        {
            if (le <= ls)
                return;

            var lunchS = day + ls;
            var lunchE = day + le;
            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            using var db = new AppDbContext();

            var toFix = db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime >= dayStart &&
                            e.StartTime < dayEnd &&
                            (e.EventType == EventType.Work || e.EventType == EventType.BusinessTrip) &&
                            e.EndTime > lunchS &&
                            e.StartTime < lunchE)
                .OrderBy(e => e.StartTime)
                .ToList();

            foreach (var ev in toFix)
            {
                bool hasLeft = ev.StartTime < lunchS;
                bool hasRight = ev.EndTime > lunchE;

                if (hasLeft && hasRight)
                {
                    db.Events.Add(new Event
                    {
                        EmployeeId = employeeId,
                        Title = ev.Title,
                        Description = ev.Description,
                        EventType = ev.EventType,
                        AllDay = false,
                        StartTime = lunchE,
                        EndTime = ev.EndTime,
                        IsAutoGenerated = ev.IsAutoGenerated,
                        AutoGeneratedForDate = ev.AutoGeneratedForDate,
                        ParentEventId = ev.ParentEventId,
                        ImportBatchId = ev.ImportBatchId,
                        HasCollision = ev.HasCollision
                    });

                    ev.AllDay = false;
                    ev.EndTime = lunchS;
                }
                else if (hasLeft)
                {
                    ev.AllDay = false;
                    ev.EndTime = lunchS;
                }
                else if (hasRight)
                {
                    ev.AllDay = false;
                    ev.StartTime = lunchE;
                }
                else
                {
                    ev.IsDeleted = true;
                }
            }

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, new[] { dayStart });
        }

        private void SplitAdjustableWorkAroundLunch(DateTime day, TimeSpan ls, TimeSpan le, int employeeId = DefaultEmployeeId)
        {
            var lunches = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType == EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .Select(e => (S: e.StartTime.TimeOfDay, E: e.EndTime.TimeOfDay))
                .Where(x => x.E > x.S)
                .ToList();

            foreach (var lunch in lunches)
                SplitAdjustableWorkAroundOneLunch(day, lunch.S, lunch.E, employeeId);

            InvalidateMonthsForDates(new[] { day });
            InvalidateWeeksForDates(new[] { day }, employeeId);
        }

        public void SplitWorkAroundExistingLunches(DateTime day, int employeeId = DefaultEmployeeId)
        {
            day = day.Date;

            bool hasLunch = GetEventsForDay(employeeId, day)
                .Any(e => !e.IsDeleted && e.EventType == EventType.Lunch);

            if (!hasLunch)
                return;

            SplitAdjustableWorkAroundLunch(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
        }

        private List<(DateTime s, DateTime e)> GetBusyIntervals(DateTime day, TimeSpan arr, TimeSpan dep, int employeeId = DefaultEmployeeId)
        {
            var winS = day + arr;
            var winE = day + dep;

            return GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType != EventType.Lunch)
                .Select(e => (s: e.StartTime < winS ? winS : e.StartTime,
                              e: e.EndTime > winE ? winE : e.EndTime))
                .Where(x => x.e > x.s)
                .OrderBy(x => x.s)
                .ToList();
        }

        private (TimeSpan ls, TimeSpan le) RefitLunch(
     DateTime day,
     TimeSpan arr,
     TimeSpan dep,
     TimeSpan desiredLs,
     TimeSpan desiredLe,
     TimeSpan fallbackLen,
     int employeeId = DefaultEmployeeId)
        {
            var len = desiredLe > desiredLs ? (desiredLe - desiredLs) : fallbackLen;
            if (dep <= arr || len <= TimeSpan.Zero)
                return (TimeSpan.Zero, TimeSpan.Zero);

            var winS = day + arr;
            var winE = day + dep;
            var busy = MergeIntervals(GetManualLunchBlockers(day, arr, dep, employeeId));

            var free = new List<(DateTime s, DateTime e)>();
            DateTime cursor = winS;

            foreach (var iv in busy)
            {
                if (iv.s > cursor)
                    free.Add((cursor, iv.s));

                if (iv.e > cursor)
                    cursor = iv.e;
            }

            if (cursor < winE)
                free.Add((cursor, winE));

            var candidates = free.Where(g => (g.e - g.s) >= len).ToList();
            if (candidates.Count == 0)
                return (TimeSpan.Zero, TimeSpan.Zero);

            var desiredStart = day + desiredLs;
            DateTime sCand;
            DateTime eCand;

            var exactFit = candidates.FirstOrDefault(g =>
                desiredLe > desiredLs &&
                desiredStart >= g.s &&
                desiredStart + len <= g.e);

            if (exactFit != default)
            {
                sCand = desiredStart;
                eCand = sCand + len;
            }
            else
            {
                var afterDesired = candidates
                    .Where(g => g.e > desiredStart)
                    .OrderBy(g => g.s)
                    .FirstOrDefault();

                if (afterDesired != default)
                {
                    sCand = desiredStart > afterDesired.s ? desiredStart : afterDesired.s;
                    if (sCand + len > afterDesired.e)
                        sCand = afterDesired.s;

                    eCand = sCand + len;
                }
                else
                {
                    var first = candidates.OrderBy(g => g.s).First();
                    sCand = first.s;
                    eCand = sCand + len;
                }
            }

            if (eCand <= sCand)
                return (TimeSpan.Zero, TimeSpan.Zero);

            return (sCand.TimeOfDay, eCand.TimeOfDay);
        }

        private bool TryCarveLunchFromInnerAuto(DateTime day, TimeSpan arr, TimeSpan dep, TimeSpan len, out TimeSpan ls, out TimeSpan le, int employeeId = DefaultEmployeeId)
        {
            ls = le = TimeSpan.Zero;

            if (dep <= arr || len <= TimeSpan.Zero)
                return false;

            var all = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType != EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .ToList();

            if (all.Count < 3)
                return false;

            var candidate = all
                .Skip(1).Take(all.Count - 2)
                .FirstOrDefault(e =>
                    e.IsAutoGenerated &&
                    e.EventType == EventType.Work &&
                    (e.EndTime - e.StartTime) >= len + TimeSpan.FromMinutes(1) &&
                    e.StartTime >= day + arr &&
                    e.EndTime <= day + dep);

            if (candidate == null)
                return false;

            var cutStart = candidate.StartTime;
            var cutEnd = candidate.StartTime + len;

            var idx = all.FindIndex(x => x.Id == candidate.Id);
            if (idx <= 0 || idx >= all.Count - 1)
                return false;

            var prevEnd = all[idx - 1].EndTime;
            var nextStart = all[idx + 1].StartTime;

            if (cutStart < prevEnd || cutEnd > nextStart)
                return false;

            using (var db = new AppDbContext())
            {
                var ev = db.Events.Single(x => x.Id == candidate.Id && x.EmployeeId == employeeId && !x.IsDeleted);
                ev.StartTime = cutEnd;
                if (ev.EndTime < ev.StartTime)
                    ev.EndTime = ev.StartTime;

                db.SaveChanges();
            }

            CreateAutoEvent(new Event
            {
                EmployeeId = employeeId,
                Title = "Oběd (auto)",
                EventType = EventType.Lunch,
                StartTime = cutStart,
                EndTime = cutEnd,
                IsAutoGenerated = true,
                AutoGeneratedForDate = day
            });

            ls = cutStart.TimeOfDay;
            le = cutEnd.TimeOfDay;
            return true;
        }

        private double TrimEndAuto(DateTime day, TimeSpan cutSpan, int employeeId)
        {
            if (cutSpan <= TimeSpan.Zero)
                return 0;

            int capMin = GetTrimEndCapacityMinutes(day, employeeId);
            if (capMin < QUANTUM_MIN)
                return 0;

            int wantMin = RoundDownToQuantum((int)Math.Round(cutSpan.TotalMinutes));
            int takeMin = Math.Min(capMin, wantMin);

            takeMin = RoundDownToQuantum(takeMin);
            if (takeMin < QUANTUM_MIN)
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);

            var arr = resolved.ArrivalTime;
            var newDep = resolved.DepartureTime - TimeSpan.FromMinutes(takeMin);

            if (newDep <= arr)
                return 0;

            var (ls, le) = GetLunchInsideBalanceWindow(day, arr, newDep, employeeId);

            SettingsService.SaveDaySettingsForDate(day, arr, newDep, ls, le, employeeId);

            if (ShouldTrackSelfTrim())
                SelfTrimStore.Add(employeeId, day, TransferEdge.End, takeMin);

            return takeMin / 60.0;
        }

        private double TrimStartAuto(DateTime day, TimeSpan cutSpan, int employeeId)
        {
            if (cutSpan <= TimeSpan.Zero)
                return 0;

            int capMin = GetTrimStartCapacityMinutes(day, employeeId);
            if (capMin < QUANTUM_MIN)
                return 0;

            int wantMin = RoundDownToQuantum((int)Math.Round(cutSpan.TotalMinutes));
            int takeMin = Math.Min(capMin, wantMin);

            takeMin = RoundDownToQuantum(takeMin);
            if (takeMin < QUANTUM_MIN)
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);

            var newArr = resolved.ArrivalTime + TimeSpan.FromMinutes(takeMin);
            var dep = resolved.DepartureTime;

            if (newArr >= dep)
                return 0;

            var (ls, le) = GetLunchInsideBalanceWindow(day, newArr, dep, employeeId);

            SettingsService.SaveDaySettingsForDate(day, newArr, dep, ls, le, employeeId);

            if (ShouldTrackSelfTrim())
                SelfTrimStore.Add(employeeId, day, TransferEdge.Start, takeMin);

            return takeMin / 60.0;
        }

        private double TrimEndAuto(DateTime day, double hours, int employeeId)
            => TrimEndAuto(day, QMinutes(ToWholeMinutes(hours)), employeeId);

        private double TrimStartAuto(DateTime day, double hours, int employeeId)
            => TrimStartAuto(day, QMinutes(ToWholeMinutes(hours)), employeeId);

        private double ExtendEndAuto(DateTime day, double hours, int employeeId)
        {
            int wantMin = RoundDownToQuantum(ToWholeMinutes(hours));
            if (wantMin < QUANTUM_MIN)
                return 0;

            int capMin = GetExtendEndCapacityMinutesByWindow(day, employeeId);
            if (capMin < QUANTUM_MIN)
                return 0;

            int takeMin = Math.Min(wantMin, capMin);
            takeMin = RoundDownToQuantum(takeMin);

            if (takeMin < QUANTUM_MIN)
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);

            var arr = resolved.ArrivalTime;
            var newDep = resolved.DepartureTime + TimeSpan.FromMinutes(takeMin);

            var softEnd = TimeSpan.FromHours(23).Add(TimeSpan.FromMinutes(55));
            if (newDep > softEnd)
                newDep = softEnd;

            if (newDep <= arr)
                return 0;

            var (ls, le) = GetLunchInsideBalanceWindow(day, arr, newDep, employeeId);

            SettingsService.SaveDaySettingsForDate(day, arr, newDep, ls, le, employeeId);

            return takeMin / 60.0;
        }

        private double ExtendStartAuto(DateTime day, double hours, int employeeId)
        {
            int wantMin = RoundDownToQuantum(ToWholeMinutes(hours));
            if (wantMin < QUANTUM_MIN)
                return 0;

            int capMin = GetExtendStartCapacityMinutesByWindow(day, employeeId);
            if (capMin < QUANTUM_MIN)
                return 0;

            int takeMin = Math.Min(wantMin, capMin);
            takeMin = RoundDownToQuantum(takeMin);

            if (takeMin < QUANTUM_MIN)
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);

            var newArr = resolved.ArrivalTime - TimeSpan.FromMinutes(takeMin);
            var dep = resolved.DepartureTime;

            if (newArr < TimeSpan.Zero)
                newArr = TimeSpan.Zero;

            var (ls, le) = GetLunchInsideBalanceWindow(day, newArr, dep, employeeId);

            SettingsService.SaveDaySettingsForDate(day, newArr, dep, ls, le, employeeId);

            return takeMin / 60.0;
        }

        private void AdjustDaySettingsAfterChange(DateTime day, int employeeId = DefaultEmployeeId)
        {
            const double EPS = 1e-6;
            var resolved = GetResolvedWindow(day, employeeId);
            var movedOut = WorkTransferReportingService.GetMovedOut(day, employeeId);
            var prev = SettingsService.GetDaySettingsForDate(day, employeeId);

            bool IsSpecial(Event e) => e.EventType != EventType.Lunch && !IsCreditedWorkTime(e);
            bool IsCredited(Event e) => IsCreditedWorkTime(e) || IsSpecial(e);

            var evs = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted)
                .OrderBy(e => e.StartTime)
                .ToList();

            var work = evs.Where(IsCreditedWorkTime).ToList();
            var specials = evs.Where(IsSpecial).ToList();
            var credited = evs.Where(IsCredited).ToList();

            if (movedOut > EPS && prev != null)
            {
                if (work.Count == 0 && specials.Count == 0)
                {
                    SettingsService.DeleteComputedDaySettingsForDate(day, employeeId);
                    ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                    return;
                }

                if (work.Count == 0 && specials.Count > 0)
                {
                    var arKeep = prev.ArrivalTime;
                    var deKeep = prev.DepartureTime;
                    SettingsService.SaveDaySettingsForDate(day, arKeep, deKeep, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                    ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                    return;
                }

                var ar = credited.Min(e => e.StartTime).TimeOfDay;
                var de = credited.Max(e => e.EndTime).TimeOfDay;

                TimeSpan lS = TimeSpan.Zero, lE = TimeSpan.Zero;
                var lunch = evs.FirstOrDefault(e => e.EventType == EventType.Lunch);
                if (lunch != null)
                {
                    var tls = lunch.StartTime.TimeOfDay;
                    var tle = lunch.EndTime.TimeOfDay;
                    if (tle > tls && tls >= ar && tle <= de)
                    {
                        lS = tls;
                        lE = tle;
                    }
                }

                SettingsService.SaveDaySettingsForDate(day, ar, de, lS, lE, employeeId);
                ReplaceLunchEvent(day, lS, lE, employeeId);
                SplitAdjustableWorkAroundLunch(day, lS, lE, employeeId);
                return;
            }

            if (work.Count == 0 && specials.Count == 0)
            {
                SettingsService.DeleteDaySettingsForDate(day, employeeId);
                ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                return;
            }

            if (work.Count == 0 && specials.Count > 0)
            {
                var arKeep = prev?.ArrivalTime ?? resolved.ArrivalTime;
                var deKeep = prev?.DepartureTime ?? resolved.DepartureTime;

                SettingsService.SaveDaySettingsForDate(day, arKeep, deKeep, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                return;
            }

            var arr = credited.Min(e => e.StartTime).TimeOfDay;
            var dep = credited.Max(e => e.EndTime).TimeOfDay;

            TimeSpan ls = TimeSpan.Zero;
            TimeSpan le = TimeSpan.Zero;

            if (prev != null && prev.LunchEnd > prev.LunchStart)
            {
                if (prev.LunchStart >= arr && prev.LunchEnd <= dep)
                {
                    ls = prev.LunchStart;
                    le = prev.LunchEnd;
                }
            }
            else if (resolved.LunchEnd > resolved.LunchStart &&
                     resolved.LunchStart >= arr &&
                     resolved.LunchEnd <= dep)
            {
                ls = resolved.LunchStart;
                le = resolved.LunchEnd;
            }

            SettingsService.SaveDaySettingsForDate(day, arr, dep, ls, le, employeeId);
            ReplaceLunchEvent(day, ls, le, employeeId);
            SplitAdjustableWorkAroundLunch(day, ls, le, employeeId);
        }

        public async Task EnsureLunchInsideWorkWindowAsync(int employeeId, DateTime day, bool callRegenerate = true)
        {
            var resolved = GetResolvedWindow(day, employeeId);
            var lunchWindow = GetLunchPolicyWindowForDay(day, employeeId);

            var arr = lunchWindow.arr;
            var dep = lunchWindow.dep;

            if (dep <= arr)
                return;

            var desiredLen = resolved.LunchEnd > resolved.LunchStart ? resolved.LunchEnd - resolved.LunchStart : TimeSpan.FromMinutes(30);

            var maxLunchLen = ParseDurationOrDefault(resolved.MaxBreakDuration, TimeSpan.FromMinutes(30));
            if (desiredLen <= TimeSpan.Zero)
                desiredLen = TimeSpan.FromMinutes(30);

            if (desiredLen > maxLunchLen)
                desiredLen = maxLunchLen;

            int targetLunchCount = GetStableTargetLunchCount(day, arr, dep, employeeId);
            if (IsFullSpecialDayWithoutLunch(day, arr, dep, targetLunchCount, desiredLen, employeeId))
            {
                SettingsService.SaveDaySettingsForDate(day, arr, dep, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                MergeAdjacentAutoWorkBlocks(day, employeeId);
                return;
            }
            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            await using (var db = new AppDbContext())
            {
                var dayAutoLunches = await db.Events
                    .Where(e =>
                        e.EmployeeId == employeeId &&
                        !e.IsDeleted &&
                        e.StartTime >= dayStart &&
                        e.StartTime < dayEnd &&
                        e.EventType == EventType.Lunch &&
                        e.IsAutoGenerated)
                    .ToListAsync();

                var invalidAutoLunches = dayAutoLunches
                    .Where(e =>
                        e.EndTime <= e.StartTime ||
                        e.StartTime.TimeOfDay < arr ||
                        e.EndTime.TimeOfDay > dep)
                    .ToList();

                if (invalidAutoLunches.Count > 0)
                {
                    db.Events.RemoveRange(invalidAutoLunches);
                    await db.SaveChangesAsync();
                    InvalidateEventCacheDays(employeeId, new[] { dayStart });
                }
            }

            var lunches = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType == EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .ToList();

            if (lunches.Count > targetLunchCount)
            {
                await using var db = new AppDbContext();

                var extraAutoLunches = lunches
                    .Skip(targetLunchCount)
                    .Where(e => e.IsAutoGenerated)
                    .ToList();

                if (extraAutoLunches.Count > 0)
                {
                    db.Events.RemoveRange(extraAutoLunches);
                    await db.SaveChangesAsync();
                    InvalidateEventCacheDays(employeeId, new[] { dayStart });
                }

                lunches = lunches
                    .Except(extraAutoLunches)
                    .OrderBy(e => e.StartTime)
                    .ToList();
            }

            if (targetLunchCount == 0)
            {
                SettingsService.SaveDaySettingsForDate(day, arr, dep, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                MergeAdjacentAutoWorkBlocks(day, employeeId);
                return;
            }

            var blockers = MergeIntervals(GetManualLunchBlockers(day, arr, dep, employeeId));

            while (lunches.Count < targetLunchCount)
            {
                DateTime desiredStart =
                    lunches.Count == 0
                        ? Max(day + arr + TimeSpan.FromHours(4), day + resolved.LunchStart)
                        : lunches.Max(x => x.EndTime) + TimeSpan.FromHours(4);

                DateTime desiredEnd = desiredStart + desiredLen;

                if (desiredEnd > day + dep)
                    break;

                while (true)
                {
                    var blocker = blockers.FirstOrDefault(x => desiredStart < x.e && x.s < desiredEnd);
                    if (blocker.e <= blocker.s)
                        break;

                    desiredStart = blocker.e;
                    desiredEnd = desiredStart + desiredLen;

                    if (desiredEnd > day + dep)
                        break;
                }

                if (desiredEnd > day + dep)
                    break;

                var autoLunch = new Event
                {
                    EmployeeId = employeeId,
                    Title = resolved.AutoEventNameLunch,
                    EventType = EventType.Lunch,
                    StartTime = desiredStart,
                    EndTime = desiredEnd,
                    IsAutoGenerated = true,
                    AutoGeneratedForDate = day
                };

                CreateAutoEvent(autoLunch);
                lunches.Add(autoLunch);
                lunches = lunches.OrderBy(e => e.StartTime).ToList();
            }

            if (lunches.Count == 0)
            {
                var (lsFallback, leFallback) = RefitLunch(
                    day,
                    arr,
                    dep,
                    resolved.LunchStart,
                    resolved.LunchEnd,
                    desiredLen,
                    employeeId);

                if (leFallback <= lsFallback)
                {
                    TryCarveLunchFromInnerAuto(day, arr, dep, desiredLen, out lsFallback, out leFallback, employeeId);
                }

                if (leFallback > lsFallback)
                {
                    ReplaceLunchEvent(day, lsFallback, leFallback, employeeId);

                    lunches = GetEventsForDay(employeeId, day)
                        .Where(e => !e.IsDeleted && e.EventType == EventType.Lunch)
                        .OrderBy(e => e.StartTime)
                        .ToList();
                }
            }

            var firstLunch = lunches.OrderBy(e => e.StartTime).FirstOrDefault();
            if (firstLunch == null)
            {
                MergeAdjacentAutoWorkBlocks(day, employeeId);
                return;
            }

            var ls = firstLunch.StartTime.TimeOfDay;
            var le = firstLunch.EndTime.TimeOfDay;

            SettingsService.SaveDaySettingsForDate(day, arr, dep, ls, le, employeeId);

            // Сначала нормализуем старые разрывы, потом снова режем вокруг lunch
            MergeAdjacentAutoWorkBlocks(day, employeeId);
            SplitAdjustableWorkAroundLunch(day, ls, le, employeeId);
        }

        public async Task EnsureLunchInsideWorkWindowAsync(DateTime day, bool callRegenerate = true)
            => await EnsureLunchInsideWorkWindowAsync(DefaultEmployeeId, day, callRegenerate);

        public record ImportBatchInfo(string Id, string Label, DateTime RangeStart, DateTime RangeEnd, int EventsCount);

        public IEnumerable<ImportBatchInfo> GetImportBatches(int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();

            var rows = db.Events
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.ImportBatchId != null)
                .GroupJoin(
                    db.ImportBatches,
                    ev => ev.ImportBatchId,
                    batch => batch.Id,
                    (ev, batches) => new { ev, batch = batches.FirstOrDefault() })
                .GroupBy(x => new { x.ev.ImportBatchId, Label = x.batch != null ? x.batch.Label : null })
                .Select(g => new
                {
                    Id = g.Key.ImportBatchId!,
                    Label = g.Key.Label,
                    RangeStart = g.Min(x => x.ev.StartTime),
                    RangeEnd = g.Max(x => x.ev.EndTime),
                    EventsCount = g.Count()
                })
                .OrderByDescending(x => x.RangeStart)
                .AsNoTracking()
                .ToList();

            return rows.Select(x =>
                new ImportBatchInfo(
                    x.Id,
                    x.Label ?? "Načtení",
                    x.RangeStart.Date,
                    x.RangeEnd.Date,
                    x.EventsCount));
        }

        private async Task<double> TransferLockedOvertimeAsync(
            DateTime lockedDay,
            double overtimeHours,
            IEnumerable<DateTime> weekDays,
            int employeeId = DefaultEmployeeId)
        {
            int targetMin = RoundDownToQuantum(ToWholeMinutes(overtimeHours));
            if (targetMin < QUANTUM_MIN)
                return 0;

            var donors = weekDays
                .Select(d => d.Date)
                .Distinct()
                .Where(d => d != lockedDay.Date)
                .Where(d => !IsLockedEdgeDay(d, employeeId))
                .Where(d => HasBalanceAdjustableWork(d, employeeId))
                .Where(d => CanTrimStart(d, employeeId) || CanTrimEnd(d, employeeId))
                .Select(d => (Day: d, CapacityMinutes: GetEdgeAutoTrimCapacityMinutes(d, employeeId)))
                .Where(x => x.CapacityMinutes >= QUANTUM_MIN)
                .OrderBy(x => x.Day)
                .ToList();

            if (donors.Count == 0)
                return 0;

            int donorCapacity = donors.Sum(x => x.CapacityMinutes);
            targetMin = Math.Min(targetMin, donorCapacity);
            targetMin = RoundDownToQuantum(targetMin);

            if (targetMin < QUANTUM_MIN)
                return 0;

            var plan = BuildEvenTrimPlan(donors, targetMin);
            if (plan.Count == 0)
                return 0;

            var preferEnd = donors.ToDictionary(x => x.Day, _ => true);
            var touched = new HashSet<DateTime>();
            int totalCut = 0;

            foreach (var kv in plan.OrderBy(x => x.Key))
            {
                var applied = ApplyPlannedTrim(kv.Key, kv.Value, employeeId, preferEnd);
                if (applied.TotalMinutes < QUANTUM_MIN)
                    continue;

                totalCut += applied.TotalMinutes;
                touched.Add(kv.Key);

                if (applied.StartMinutes >= QUANTUM_MIN)
                {
                    WorkTransferReportingService.AddTransfer(
                        kv.Key,
                        lockedDay,
                        applied.StartMinutes,
                        TransferEdge.Start,
                        employeeId);
                }

                if (applied.EndMinutes >= QUANTUM_MIN)
                {
                    WorkTransferReportingService.AddTransfer(
                        kv.Key,
                        lockedDay,
                        applied.EndMinutes,
                        TransferEdge.End,
                        employeeId);
                }
            }

            var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

            foreach (var d in touched.OrderBy(x => x))
            {
                await gen.RegenerateDailyEventsAsync(d, preserveUserSettings: true, ensureLunchAfterGeneration: false);
                await EnsureLunchInsideWorkWindowAsync(employeeId, d, callRegenerate: false);
            }

            if (touched.Count > 0)
                NotifyAutoEventsGenerated();

            return totalCut / 60.0;
        }

        private async Task<double> TrimAutoSymmetricAsync(DateTime day, double hours, bool callRegenerate = true, int employeeId = DefaultEmployeeId)
        {
            int toCutMin = RoundDownToQuantum(ToWholeMinutes(hours));
            if (toCutMin <= 0)
                return 0;

            int cut = 0;

            while (toCutMin - cut >= QUANTUM_MIN && (CanTrimStart(day, employeeId) || CanTrimEnd(day, employeeId)))
            {
                int left = toCutMin - cut;

                int endWant = RoundUpToQuantum(left / 2);
                if (endWant > left) endWant = left;
                int startWant = left - endWant;

                double c1 = 0, c2 = 0;

                if (endWant > 0 && CanTrimEnd(day, employeeId))
                    c1 = TrimEndAuto(day, TimeSpan.FromMinutes(endWant), employeeId);

                if (startWant > 0 && CanTrimStart(day, employeeId))
                    c2 = TrimStartAuto(day, TimeSpan.FromMinutes(startWant), employeeId);

                int got = RoundDownToQuantum((int)Math.Round((c1 + c2) * 60.0));

                if (got <= 0)
                {
                    if (CanTrimEnd(day, employeeId))
                        got = RoundDownToQuantum((int)Math.Round(TrimEndAuto(day, QMinutes(QUANTUM_MIN), employeeId) * 60.0));

                    if (got <= 0 && CanTrimStart(day, employeeId))
                        got = RoundDownToQuantum((int)Math.Round(TrimStartAuto(day, QMinutes(QUANTUM_MIN), employeeId) * 60.0));

                    if (got <= 0)
                        break;
                }

                cut += got;
            }

            if (callRegenerate)
            {
                var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);
                await gen.RegenerateDailyEventsAsync(day, preserveUserSettings: true, ensureLunchAfterGeneration: false);
            }

            await EnsureLunchInsideWorkWindowAsync(employeeId, day, callRegenerate: false);
            NotifyAutoEventsGenerated();

            return cut / 60.0;
        }

        public async Task TrimOvertimeByAutoBlocksAsync(int employeeId, DateTime day, bool preserveUserSettings = false)
        {
            int overMin = GetBalanceOverMinutes(day, employeeId);

            if (overMin < QUANTUM_MIN)
                return;

            await TrimAutoSymmetricAsync(day, overMin / 60.0, callRegenerate: false, employeeId);
        }

        public async Task TrimOvertimeByAutoBlocksAsync(DateTime day, bool preserveUserSettings = false)
            => await TrimOvertimeByAutoBlocksAsync(DefaultEmployeeId, day, preserveUserSettings);

        private sealed class WeekDayMeta
        {
            public DateTime Day;
            public double Extra;
            public bool Locked;
            public bool CanStart;
            public bool CanEnd;
        }

        private async Task<PoolRestoreResult> RollbackSelfTrimPoolForWeekAsync(
            IReadOnlyList<DateTime> scopeDays,
            int employeeId = DefaultEmployeeId)
        {
            int weekUnderMin = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .Sum(d => GetBalanceUnderMinutes(d, employeeId));

            weekUnderMin = RoundDownToQuantum(weekUnderMin);
            if (weekUnderMin < QUANTUM_MIN)
                return new PoolRestoreResult();

            var entries = SelfTrimStore.GetScopeSnapshot(employeeId, scopeDays)
                .OrderByDescending(x => x.Minutes)
                .ThenBy(x => x.Day)
                .ThenBy(x => x.Edge == TransferEdge.End ? 0 : 1)
                .ToList();

            if (entries.Count == 0)
                return new PoolRestoreResult();

            int left = weekUnderMin;
            int restored = 0;
            var touched = new HashSet<DateTime>();
            var restoredByDay = new Dictionary<DateTime, int>();

            foreach (var entry in entries)
            {
                if (left < QUANTUM_MIN)
                    break;

                int want = Math.Min(left, entry.Minutes);
                want = RoundDownToQuantum(want);

                if (want < QUANTUM_MIN)
                    continue;

                var putBackHours = RestoreTransferredMinutes(entry.Day, want, entry.Edge, employeeId);
                int putBackMin = RoundDownToQuantum((int)Math.Round(putBackHours * 60.0));

                if (putBackMin < QUANTUM_MIN)
                    continue;

                restored += putBackMin;
                left -= putBackMin;
                touched.Add(entry.Day.Date);
                AddRestoredMinutes(restoredByDay, entry.Day, putBackMin);

                SelfTrimStore.Consume(employeeId, entry.Day, entry.Edge, putBackMin);
            }

            if (restored >= QUANTUM_MIN)
            {
                var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

                foreach (var d in touched.OrderBy(x => x))
                {
                    await gen.RegenerateDailyEventsAsync(d, preserveUserSettings: true, ensureLunchAfterGeneration: false);
                    await EnsureLunchInsideWorkWindowAsync(employeeId, d, callRegenerate: false);
                }

                NotifyAutoEventsGenerated();
            }

            return new PoolRestoreResult
            {
                RestoredMinutes = restored,
                TouchedDays = touched,
                MinutesByDay = restoredByDay
            };
        }

        private List<DateTime> GetMonthPartialScopeDays(int year, int month)
        {
            var monthDays = GetMonthWorkdays(year, month);
            if (monthDays.Count == 0)
                return new List<DateTime>();

            var orderedMonthGroups = monthDays
                .GroupBy(GetIsoWeekKey)
                .Select(g => g.OrderBy(d => d).ToList())
                .Where(g => g.Count > 0)
                .OrderBy(g => g.First())
                .ToList();

            var partialGroups = orderedMonthGroups
                .Where(g => IsPartialMonthWeek(GetIsoWeekKey(g[0]), month))
                .ToList();

            if (partialGroups.Count == 0)
                return new List<DateTime>();

            if (partialGroups.Count >= 2)
            {
                return partialGroups
                    .SelectMany(g => g)
                    .Select(d => d.Date)
                    .Distinct()
                    .OrderBy(d => d)
                    .ToList();
            }

            var fallbackScope = BuildSinglePartialFallbackScope(
                orderedMonthGroups,
                partialGroups[0],
                month);

            if (fallbackScope != null && fallbackScope.Count >= 2)
            {
                return fallbackScope
                    .Select(d => d.Date)
                    .Distinct()
                    .OrderBy(d => d)
                    .ToList();
            }

            return partialGroups[0]
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();
        }

        private bool ShouldRebalanceMonthPartialScope(int year, int month, IEnumerable<DateTime> changedDays)
        {
            var changed = changedDays
                .Select(d => d.Date)
                .Distinct()
                .ToHashSet();

            var partialScopeDays = GetMonthPartialScopeDays(year, month);

            if (partialScopeDays.Count == 0)
                return false;

            return partialScopeDays.Any(changed.Contains);
        }

        private async Task BalanceOneWeekAsync(List<DateTime> weekDays, int employeeId = DefaultEmployeeId)
        {
            const double EPS = 1e-6;
            var scopeDays = weekDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            if (scopeDays.Count == 0)
                return;

            var calc = new WorkingHoursCalculatorService();

            Dictionary<DateTime, int> BuildActualExtraMinByDay()
                => scopeDays.ToDictionary(
                    d => d,
                    d => GetBalanceOverMinutes(d, employeeId));

            Dictionary<DateTime, int> BuildActualUnderMinByDay()
                => scopeDays.ToDictionary(
                    d => d,
                    d => GetBalanceUnderMinutes(d, employeeId));

            int GetActualWeekNetMin()
                => BuildActualExtraMinByDay().Values.Sum() - BuildActualUnderMinByDay().Values.Sum();

            var meta = scopeDays.ToDictionary(d => d, d =>
            {
                var balance = GetDailyBalanceMinutes(d, employeeId);

                return new WeekDayMeta
                {
                    Day = d,
                    Extra = balance.OverMinutes / 60.0,
                    Locked = IsLockedEdgeDay(d, employeeId),
                    CanStart = CanTrimStart(d, employeeId),
                    CanEnd = CanTrimEnd(d, employeeId)
                };
            });

            foreach (var d in scopeDays.OrderBy(x => x))
            {
                var movedIn = WorkTransferReportingService.GetMovedIn(d, employeeId);
                if (movedIn <= EPS)
                    continue;

                var actual = meta[d];
                int needBackMin = RoundDownToQuantum((int)Math.Round((movedIn - actual.Extra) * 60.0));

                if (needBackMin < QUANTUM_MIN)
                    continue;

                var rb = await RollbackTransfersForLockedDayAsync(d, needBackMin, employeeId);
                if (rb.RestoredMinutes > 0)
                    RefreshWeekMeta(meta, scopeDays, calc, employeeId);
            }

            var selfTrimRollback = await RollbackSelfTrimPoolForWeekAsync(scopeDays, employeeId);
            if (selfTrimRollback.RestoredMinutes >= QUANTUM_MIN)
                RefreshWeekMeta(meta, scopeDays, calc, employeeId);

            var transferRollback = await RollbackTransferPoolForScopeAsync(scopeDays, employeeId);
            if (transferRollback.RestoredMinutes >= QUANTUM_MIN)
                RefreshWeekMeta(meta, scopeDays, calc, employeeId);

            RefreshWeekMeta(meta, scopeDays, calc, employeeId);

            for (int pass = 1; pass <= 4; pass++)
            {
                int weekNetMin = GetActualWeekNetMin();
                if (Math.Abs(weekNetMin) < QUANTUM_MIN)
                    break;

                bool progressed = false;

                if (weekNetMin > 0)
                {
                    var unlockedDays = scopeDays
                        .Where(d => !IsLockedEdgeDay(d, employeeId))
                        .OrderBy(d => d)
                        .ToList();

                    if (unlockedDays.Count > 0)
                    {
                        int trimmed = await TrimWeekRoundRobinAsync(unlockedDays, weekNetMin, employeeId);
                        if (trimmed >= QUANTUM_MIN)
                            progressed = true;
                    }

                    var actualExtraAfterTrim = BuildActualExtraMinByDay();
                    int remainingPositiveNetMin = GetActualWeekNetMin();

                    foreach (var d in scopeDays.OrderBy(x => x))
                    {
                        if (remainingPositiveNetMin < QUANTUM_MIN)
                            break;

                        int lockedExtraMin = actualExtraAfterTrim.TryGetValue(d, out var val) ? val : 0;
                        if (lockedExtraMin < QUANTUM_MIN)
                            continue;

                        int transferNeedMin = Math.Min(lockedExtraMin, remainingPositiveNetMin);
                        transferNeedMin = RoundDownToQuantum(transferNeedMin);
                        if (transferNeedMin < QUANTUM_MIN)
                            continue;

                        var transferred = await TransferLockedOvertimeAsync(
                            d,
                            transferNeedMin / 60.0,
                            scopeDays,
                            employeeId);

                        int transferredMin = RoundDownToQuantum(ToWholeMinutes(transferred));

                        if (transferredMin >= QUANTUM_MIN)
                        {
                            progressed = true;
                            actualExtraAfterTrim = BuildActualExtraMinByDay();
                            remainingPositiveNetMin = GetActualWeekNetMin();
                        }
                    }
                }
                else
                {
                    int restored = await RestoreWeekRoundRobinAsync(
                        scopeDays,
                        Math.Abs(weekNetMin),
                        employeeId,
                        skipDays: null);

                    if (restored >= QUANTUM_MIN)
                        progressed = true;
                }

                RefreshWeekMeta(meta, scopeDays, calc, employeeId);

                if (!progressed)
                    break;
            }

            foreach (var d in scopeDays)
                ReconcileSelfTrimPoolForDay(d, employeeId);
        }

        private void ResetBalancingArtifactsForScope(IEnumerable<DateTime> scopeDays, int employeeId)
        {
            var days = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            if (days.Count == 0)
                return;

            SelfTrimStore.ClearDates(employeeId, days);

            var transfers = days
                .SelectMany(d =>
                    WorkTransferReportingService.GetTransfersFrom(d, employeeId)
                        .Concat(WorkTransferReportingService.GetTransfersTo(d, employeeId)))
                .GroupBy(x => new
                {
                    From = x.FromDay.Date,
                    To = x.ToDay.Date,
                    x.Edge,
                    x.Minutes
                })
                .Select(g => g.First())
                .ToList();

            foreach (var tr in transfers)
                WorkTransferReportingService.RemoveEntry(tr);
        }

        private string BuildBalanceFingerprint(IEnumerable<DateTime> scopeDays, int employeeId)
        {
            var days = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            var sb = new StringBuilder();

            foreach (var day in days)
            {
                sb.AppendLine($"DAY|{day:yyyy-MM-dd}");

                var evs = GetEventsForDay(employeeId, day)
                    .Where(e => !e.IsDeleted)
                    .OrderBy(e => e.StartTime)
                    .ThenBy(e => e.EndTime)
                    .ThenBy(e => e.EventType)
                    .ThenBy(e => e.IsAutoGenerated)
                    .ThenBy(e => e.Title ?? string.Empty)
                    .ToList();

                foreach (var e in evs)
                {
                    sb.AppendLine(
                        $"EV|{e.EventType}|{e.IsAutoGenerated}|{e.StartTime:O}|{e.EndTime:O}|{e.ParentEventId}|{e.AutoGeneratedForDate:yyyy-MM-dd}|{e.Title}|{e.ImportBatchId}");
                }

                var ds = SettingsService.GetDaySettingsForDate(day, employeeId);
                if (ds == null)
                    sb.AppendLine("SET|null");
                else
                    sb.AppendLine($"SET|{ds.ArrivalTime}|{ds.DepartureTime}|{ds.LunchStart}|{ds.LunchEnd}");

                sb.AppendLine($"SELF|{SelfTrimStore.GetTotal(employeeId, day)}");

                foreach (var tr in WorkTransferReportingService.GetTransfersFrom(day, employeeId)
                             .OrderBy(x => x.ToDay)
                             .ThenBy(x => x.Edge)
                             .ThenBy(x => x.Minutes))
                {
                    sb.AppendLine($"TFROM|{tr.FromDay:yyyy-MM-dd}|{tr.ToDay:yyyy-MM-dd}|{tr.Edge}|{tr.Minutes}");
                }

                foreach (var tr in WorkTransferReportingService.GetTransfersTo(day, employeeId)
                             .OrderBy(x => x.FromDay)
                             .ThenBy(x => x.Edge)
                             .ThenBy(x => x.Minutes))
                {
                    sb.AppendLine($"TTO|{tr.FromDay:yyyy-MM-dd}|{tr.ToDay:yyyy-MM-dd}|{tr.Edge}|{tr.Minutes}");
                }
            }

            using var sha = SHA256.Create();
            var bytes = Encoding.UTF8.GetBytes(sb.ToString());
            var hash = sha.ComputeHash(bytes);
            return Convert.ToHexString(hash);
        }

        private List<DateTime> GetRelatedRebalanceScopeDays(List<DateTime> weekDays)
        {
            var result = weekDays
                .Select(d => d.Date)
                .Distinct()
                .ToHashSet();

            var months = result
                .Select(d => (d.Year, d.Month))
                .Distinct()
                .ToList();

            foreach (var m in months)
            {
                if (!ShouldRebalanceMonthPartialScope(m.Year, m.Month, result))
                    continue;

                foreach (var d in GetMonthPartialScopeDays(m.Year, m.Month))
                    result.Add(d.Date);
            }

            return result
                .OrderBy(d => d)
                .ToList();
        }

        private async Task BalanceWeekPipelineUntilStableAsync(List<DateTime> weekDays, int employeeId)
        {
            var baseWeekDays = weekDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            if (baseWeekDays.Count == 0)
                return;

            var fullScopeDays = GetRelatedRebalanceScopeDays(baseWeekDays);

            var affectedMonths = baseWeekDays
                .Select(d => (d.Year, d.Month))
                .Distinct()
                .OrderBy(x => x.Year)
                .ThenBy(x => x.Month)
                .ToList();

            using var _settingsCache = SettingsService.BeginReadCache(employeeId, fullScopeDays);
            using var _cache = BeginEventReadCache(employeeId, fullScopeDays);

            var seenFingerprints = new HashSet<string>(StringComparer.Ordinal);

            for (int pass = 1; pass <= 4; pass++)
            {
                WorkTransferReportingService.ResetWeek(baseWeekDays);
                ResetBalancingArtifactsForScope(baseWeekDays, employeeId);

                await BalanceOneWeekAsync(baseWeekDays, employeeId);

                foreach (var m in affectedMonths)
                {
                    if (!ShouldRebalanceMonthPartialScope(m.Year, m.Month, baseWeekDays))
                        continue;

                    var partialScope = GetMonthPartialScopeDays(m.Year, m.Month);
                    if (partialScope.Count == 0)
                        continue;

                    WorkTransferReportingService.ResetWeek(partialScope);
                    ResetBalancingArtifactsForScope(partialScope, employeeId);

                    await BalanceMonthPartialScopesAsync(m.Year, m.Month, employeeId);
                }

                foreach (var m in affectedMonths)
                    await PostNormalizeMonthAsync(m.Year, m.Month, employeeId);

                foreach (var d in fullScopeDays)
                    ReconcileSelfTrimPoolForDay(d, employeeId);

                var fp = BuildBalanceFingerprint(fullScopeDays, employeeId);

                if (!seenFingerprints.Add(fp))
                    break;
            }

            SaveBalancedWeeksForDates(fullScopeDays, employeeId);
        }

        private async Task BalanceMonthPipelineUntilStableAsync(int year, int month, int employeeId)
        {
            var monthDays = GetMonthWorkdays(year, month)
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            if (monthDays.Count == 0)
                return;

            var fullScopeDays = monthDays
                .Concat(GetMonthPartialScopeDays(year, month))
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            using var _settingsCache = SettingsService.BeginReadCache(employeeId, fullScopeDays);
            using var _cache = BeginEventReadCache(employeeId, fullScopeDays);

            var seenFingerprints = new HashSet<string>(StringComparer.Ordinal);

            for (int pass = 1; pass <= 4; pass++)
            {
                WorkTransferReportingService.ResetWeek(monthDays);
                ResetBalancingArtifactsForScope(monthDays, employeeId);

                var weekGroups = monthDays
                    .GroupBy(GetIsoWeekKey)
                    .OrderBy(g => g.Key.IsoYear)
                    .ThenBy(g => g.Key.IsoWeek)
                    .ToList();

                foreach (var g in weekGroups)
                    await BalanceOneWeekAsync(g.OrderBy(d => d).ToList(), employeeId);

                if (ShouldRebalanceMonthPartialScope(year, month, monthDays))
                {
                    var partialScope = GetMonthPartialScopeDays(year, month);
                    if (partialScope.Count > 0)
                    {
                        WorkTransferReportingService.ResetWeek(partialScope);
                        ResetBalancingArtifactsForScope(partialScope, employeeId);

                        await BalanceMonthPartialScopesAsync(year, month, employeeId);
                    }
                }

                await PostNormalizeMonthAsync(year, month, employeeId);

                foreach (var d in fullScopeDays)
                    ReconcileSelfTrimPoolForDay(d, employeeId);

                var fp = BuildBalanceFingerprint(fullScopeDays, employeeId);

                if (!seenFingerprints.Add(fp))
                    break;
            }

            SaveBalancedWeeksForDates(fullScopeDays, employeeId);
        }

        public async Task BalanceEventsForMonthAsync(int year, int month, Func<string, Task<bool>> askCollision, int employeeId = DefaultEmployeeId)
        {
            var monthDays = GetMonthWorkdays(year, month);
            if (monthDays.Count == 0)
                return;

            await BalanceMonthPipelineUntilStableAsync(year, month, employeeId);
        }

        public async Task BalanceWeekForDateAsync(DateTime anyDate, int employeeId = DefaultEmployeeId)
        {
            if (_isBalancingNow)
            {
                return;
            }

            try
            {
                _isBalancingNow = true;

                var days = GetWeekWorkdays(anyDate)
                    .Select(d => d.Date)
                    .ToList();

                if (days.Count == 0)
                {
                    return;
                }

                await BalanceWeekPipelineUntilStableAsync(days, employeeId);

            }
            finally
            {
                _isBalancingNow = false;
            }
        }

        public async Task BalanceForChangedRangeAsync(DateTime startIncl, DateTime endIncl, int employeeId = DefaultEmployeeId)
        {
            if (_isBalancingNow)
            {           
                return;
            }

            try
            {
                _isBalancingNow = true;

                var changedDays = Enumerable.Range(0, (endIncl.Date - startIncl.Date).Days + 1)
                    .Select(i => startIncl.Date.AddDays(i))
                    .Where(IsWorkday)
                    .Select(d => d.Date)
                    .ToList();

                if (changedDays.Count == 0)
                    return;

                var touchedWeeks = changedDays
                    .Select(d => new
                    {
                        AnyDay = d,
                        IsoYear = System.Globalization.ISOWeek.GetYear(d),
                        IsoWeek = System.Globalization.ISOWeek.GetWeekOfYear(d)
                    })
                    .GroupBy(x => (x.IsoYear, x.IsoWeek))
                    .Select(g => g.First().AnyDay)
                    .ToList();

                foreach (var anyDay in touchedWeeks)
                {
                    var weekDays = GetWeekWorkdays(anyDay)
                        .Select(d => d.Date)
                        .ToList();

                    var isoYear = System.Globalization.ISOWeek.GetYear(anyDay);
                    var isoWeek = System.Globalization.ISOWeek.GetWeekOfYear(anyDay);

                    await BalanceWeekPipelineUntilStableAsync(weekDays, employeeId);
                }
            }
            finally
            {
                _isBalancingNow = false;
            }
        }

        private async Task PostNormalizeMonthAsync(int year, int month, int employeeId = DefaultEmployeeId)
        {
            const double EPS = 1e-6;

            var first = new DateTime(year, month, 1);
            var last = new DateTime(year, month, DateTime.DaysInMonth(year, month));

            var monthDays = Enumerable.Range(0, (last - first).Days + 1)
                .Select(i => first.AddDays(i))
                .Where(d => d.DayOfWeek is not DayOfWeek.Saturday and not DayOfWeek.Sunday && !HolidayHelper.IsCzechHoliday(d))
                .ToList();

            foreach (var day in monthDays)
            {
                var all = GetEventsForDay(employeeId, day);
                var balance = GetDailyBalanceMinutes(day, employeeId);

                var resolved = GetResolvedWindow(day, employeeId);
                var lunchWindow = GetLunchPolicyWindowForDay(day, employeeId);
                var arrForLunch = lunchWindow.arr;
                var depForLunch = lunchWindow.dep;

                var desiredLenForLunch =
                    resolved.LunchEnd > resolved.LunchStart
                        ? resolved.LunchEnd - resolved.LunchStart
                        : TimeSpan.FromMinutes(30);

                var maxLunchLenForLunch = ParseDurationOrDefault(resolved.MaxBreakDuration, TimeSpan.FromMinutes(30));
                if (desiredLenForLunch <= TimeSpan.Zero)
                    desiredLenForLunch = TimeSpan.FromMinutes(30);
                if (desiredLenForLunch > maxLunchLenForLunch)
                    desiredLenForLunch = maxLunchLenForLunch;

                int targetLunchCountForDay = GetStableTargetLunchCount(day, arrForLunch, depForLunch, employeeId);

                bool fullSpecialDayWithoutLunch = IsFullSpecialDayWithoutLunch(
                    day,
                    arrForLunch,
                    depForLunch,
                    targetLunchCountForDay,
                    desiredLenForLunch,
                    employeeId);

                if (fullSpecialDayWithoutLunch)
                {
                    ReplaceLunchEvent(day, TimeSpan.Zero, TimeSpan.Zero, employeeId);
                    MergeAdjacentAutoWorkBlocks(day, employeeId);
                    continue;
                }

                bool hasLunch = all.Any(e => e.EventType == EventType.Lunch &&
                                             !e.IsDeleted &&
                                             (e.EndTime - e.StartTime) >= TimeSpan.FromMinutes(5));

                bool hasAnyWorkLike = all.Any(e => !e.IsDeleted && IsCreditedWorkTime(e));

                if (!hasLunch && hasAnyWorkLike && targetLunchCountForDay > 0)
                {
                    await EnsureLunchInsideWorkWindowAsync(employeeId, day);

                    all = GetEventsForDay(employeeId, day);
                    balance = GetDailyBalanceMinutes(day, employeeId);

                    hasLunch = all.Any(e => e.EventType == EventType.Lunch &&
                                            !e.IsDeleted &&
                                            (e.EndTime - e.StartTime) >= TimeSpan.FromMinutes(5));
                }

                double total = balance.CreditedMinutes / 60.0;
                bool canStart = CanExtendStart(day, employeeId);
                bool canEnd = CanExtendEnd(day, employeeId);
                var movedOut = WorkTransferReportingService.GetMovedOut(day, employeeId);
                var movedIn = WorkTransferReportingService.GetMovedIn(day, employeeId);

                if (movedOut > EPS ||
                    movedIn > EPS ||
                    balance.OverMinutes >= QUANTUM_MIN ||
                    balance.UnderMinutes >= QUANTUM_MIN)
                {
                    ReconcileSelfTrimPoolForDay(day, employeeId);
                    continue;
                }

                if (total < 8.0 - EPS && (canStart || canEnd) && movedOut <= EPS && movedIn <= EPS)
                {
                    double need = 8.0 - total;
                    RestoreUnderworkSmart(day, need, employeeId);

                    var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);
                    await gen.RegenerateDailyEventsAsync(day, ensureLunchAfterGeneration: false);
                    await EnsureLunchInsideWorkWindowAsync(employeeId, day);

                    all = GetEventsForDay(employeeId, day);
                    balance = GetDailyBalanceMinutes(day, employeeId);
                    total = balance.CreditedMinutes / 60.0;
                }

                if (!hasLunch && hasAnyWorkLike && targetLunchCountForDay > 0 && (canStart || canEnd) && movedOut <= EPS && movedIn <= EPS)
                {
                    resolved = GetResolvedWindow(day, employeeId);
                    var arr = resolved.ArrivalTime;
                    var dep = resolved.DepartureTime;
                    var desiredLen = resolved.LunchEnd > resolved.LunchStart
                        ? resolved.LunchEnd - resolved.LunchStart
                        : TimeSpan.FromMinutes(30);

                    var (ls, le) = RefitLunch(day, arr, dep, resolved.LunchStart, resolved.LunchEnd, desiredLen, employeeId);

                    if (le <= ls)
                    {
                        if (!TryCarveLunchFromInnerAuto(day, arr, dep, desiredLen, out ls, out le, employeeId))
                        {
                            if (canEnd)
                            {
                                ExtendEndAuto(day, desiredLen.TotalHours, employeeId);
                                var us2 = GetResolvedWindow(day, employeeId);
                                dep = us2.DepartureTime;
                                (ls, le) = RefitLunch(day, arr, dep, resolved.LunchStart, resolved.LunchEnd, desiredLen, employeeId);
                            }
                            else if (canStart)
                            {
                                ExtendStartAuto(day, desiredLen.TotalHours, employeeId);
                                var us2 = GetResolvedWindow(day, employeeId);
                                arr = us2.ArrivalTime;
                                (ls, le) = RefitLunch(day, arr, dep, resolved.LunchStart, resolved.LunchEnd, desiredLen, employeeId);
                            }
                        }
                    }

                    if (le > ls)
                    {
                        SettingsService.SaveDaySettingsForDate(day, arr, dep, ls, le, employeeId);
                        ReplaceLunchEvent(day, ls, le, employeeId);

                        await EnsureLunchInsideWorkWindowAsync(employeeId, day);

                        all = GetEventsForDay(employeeId, day);
                        balance = GetDailyBalanceMinutes(day, employeeId);
                        total = balance.CreditedMinutes / 60.0;

                        if (total < 8.0 - EPS)
                        {
                            double need = 8.0 - total;
                            RestoreUnderworkSmart(day, need, employeeId);

                            var gen2 = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);
                            await gen2.RegenerateDailyEventsAsync(day, ensureLunchAfterGeneration: false);
                            await EnsureLunchInsideWorkWindowAsync(employeeId, day);
                        }
                    }
                }
            }
        }

        private void InvalidateWeeksForDates(IEnumerable<DateTime> dates, int employeeId)
        {
            if (_isBalancingNow)
                return;

            var weeks = dates
                .Select(d => (Year: System.Globalization.ISOWeek.GetYear(d),
                              Week: System.Globalization.ISOWeek.GetWeekOfYear(d)))
                .Distinct()
                .ToList();

            foreach (var w in weeks)
                Helpers.WeekBalanceStore.Invalidate(employeeId, w.Year, w.Week);
        }

        public async Task CreateImportBatchAsync(ImportBatch batch)
        {
            await using var db = new AppDbContext();
            db.ImportBatches.Add(batch);
            await db.SaveChangesAsync();
        }

        private bool MustSplitManualEightHourWork(Event ev)
        {
            if (ev.IsAutoGenerated)
                return false;

            if (ev.ImportBatchId != null)
                return false;

            if (ev.EventType != EventType.Work)
                return false;

            if (ev.AllDay)
                return false;

            if (ev.StartTime.Date != ev.EndTime.Date)
                return false;

            var duration = ev.EndTime - ev.StartTime;
            return duration >= TimeSpan.FromHours(8);
        }

        private void SaveBalancedWeeksForDates(IEnumerable<DateTime> dates, int employeeId)
        {
            var weekKeys = dates
                .Select(d => new
                {
                    AnyDay = d.Date,
                    ISOYear = System.Globalization.ISOWeek.GetYear(d),
                    ISOWeek = System.Globalization.ISOWeek.GetWeekOfYear(d)
                })
                .GroupBy(x => (x.ISOYear, x.ISOWeek))
                .Select(g => g.First())
                .ToList();

            foreach (var w in weekKeys)
            {
                var fp = Helpers.BalanceFingerprint.ForWeek(this, employeeId, w.AnyDay);

                Helpers.WeekBalanceStore.Save(employeeId, w.ISOYear, w.ISOWeek, fp);
            }
        }

        private async Task FillWeeklyUnderworkAsync(
            List<DateTime> weekDays,
            int employeeId = DefaultEmployeeId,
            ISet<DateTime>? skipDays = null,
            int maxRestoreMinutes = int.MaxValue)
        {
            var touched = new HashSet<DateTime>();

            var skip = skipDays?.Select(d => d.Date).ToHashSet()
                       ?? new HashSet<DateTime>();

            int leftToRestoreMin = maxRestoreMinutes == int.MaxValue
                ? int.MaxValue
                : RoundDownToQuantum(maxRestoreMinutes);

            foreach (var rawDay in weekDays.OrderBy(d => d))
            {
                var day = rawDay.Date;

                if (leftToRestoreMin != int.MaxValue && leftToRestoreMin < QUANTUM_MIN)
                    break;

                if (skip.Contains(day))
                    continue;

                int underMin = GetBalanceUnderMinutes(day, employeeId);

                bool canStart = CanExtendStart(day, employeeId);
                bool canEnd = CanExtendEnd(day, employeeId);

                if (underMin < QUANTUM_MIN)
                    continue;

                if (leftToRestoreMin != int.MaxValue)
                {
                    underMin = Math.Min(underMin, leftToRestoreMin);
                    underMin = RoundDownToQuantum(underMin);

                    if (underMin < QUANTUM_MIN)
                        continue;
                }

                bool hasOutgoingTransfers = WorkTransferReportingService.GetTransfersFrom(day, employeeId).Any();
                bool hasIncomingTransfers = WorkTransferReportingService.GetTransfersTo(day, employeeId).Any();

                if (hasOutgoingTransfers && !hasIncomingTransfers)
                    continue;

                double placed = RestoreUnderworkSmart(day, underMin / 60.0, employeeId);
                int placedMin = RoundDownToQuantum(ToWholeMinutes(placed));

                if (placedMin >= QUANTUM_MIN)
                {
                    touched.Add(day);

                    if (leftToRestoreMin != int.MaxValue)
                        leftToRestoreMin = Math.Max(0, leftToRestoreMin - placedMin);
                }
            }

            var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

            foreach (var day in touched.OrderBy(d => d))
            {
                await gen.RegenerateDailyEventsAsync(day, preserveUserSettings: true, ensureLunchAfterGeneration: false);
                await EnsureLunchInsideWorkWindowAsync(employeeId, day, callRegenerate: false);
            }

            if (touched.Count > 0)
                NotifyAutoEventsGenerated();
        }

        private const double BALANCE_EPS = 1e-6;

        public bool IsScopeReallyBalanced(IEnumerable<DateTime> days, int employeeId = DefaultEmployeeId)
        {
            var totals = GetScopeTotals(days, employeeId);
            return totals.extra <= BALANCE_EPS && totals.under <= BALANCE_EPS;
        }

        private double RestoreTransferredMinutes(
            DateTime day,
            int minutes,
            TransferEdge edge,
            int employeeId)
        {
            if (minutes <= 0)
                return 0;

            var span = TimeSpan.FromMinutes(RoundDownToQuantum(minutes));
            if (span <= TimeSpan.Zero)
                return 0;

            return edge switch
            {
                TransferEdge.Start => ExtendStartAuto(day, span.TotalHours, employeeId),
                TransferEdge.End => ExtendEndAuto(day, span.TotalHours, employeeId),
                _ => 0
            };
        }

        private async Task<RollbackResult> RollbackTransfersForLockedDayAsync(
            DateTime lockedDay,
            int neededMinutes,
            int employeeId = DefaultEmployeeId)
        {
            int left = RoundDownToQuantum(neededMinutes);
            if (left < QUANTUM_MIN)
                return new RollbackResult();

            var incoming = WorkTransferReportingService
                .GetTransfersTo(lockedDay, employeeId)
                .OrderByDescending(x => x.Minutes)
                .ThenBy(x => x.FromDay)
                .ToList();

            if (incoming.Count == 0)
                return new RollbackResult();

            var touched = new HashSet<DateTime>();
            var restoredByDay = new Dictionary<DateTime, int>();
            int restored = 0;

            foreach (var entry in incoming)
            {
                if (left < QUANTUM_MIN)
                    break;

                int want = Math.Min(left, entry.Minutes);
                want = RoundDownToQuantum(want);
                if (want < QUANTUM_MIN)
                    continue;

                var putBackHours = RestoreTransferredMinutes(
                    entry.FromDay,
                    want,
                    entry.Edge,
                    employeeId);

                int putBackMin = RoundDownToQuantum((int)Math.Round(putBackHours * 60.0));
                if (putBackMin < QUANTUM_MIN)
                    continue;

                restored += putBackMin;
                left -= putBackMin;
                touched.Add(entry.FromDay.Date);
                AddRestoredMinutes(restoredByDay, entry.FromDay, putBackMin);

                WorkTransferReportingService.RemoveEntry(entry);

                int remainder = entry.Minutes - putBackMin;
                remainder = RoundDownToQuantum(remainder);

                if (remainder >= QUANTUM_MIN)
                {
                    WorkTransferReportingService.AddTransfer(
                        entry.FromDay.Date,
                        entry.ToDay.Date,
                        remainder,
                        entry.Edge,
                        employeeId);
                }
            }

            var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

            foreach (var d in touched.OrderBy(x => x))
            {
                await gen.RegenerateDailyEventsAsync(d, preserveUserSettings: true, ensureLunchAfterGeneration: false);
                await EnsureLunchInsideWorkWindowAsync(employeeId, d, callRegenerate: false);
            }

            if (restored > 0)
                NotifyAutoEventsGenerated();

            return new RollbackResult
            {
                RestoredMinutes = restored,
                TouchedDays = touched,
                MinutesByDay = restoredByDay
            };
        }

        private void RefreshWeekMeta(
            Dictionary<DateTime, WeekDayMeta> meta,
            IEnumerable<DateTime> scopeDays,
            WorkingHoursCalculatorService calc,
            int employeeId)
        {
            foreach (var wd in scopeDays.Select(x => x.Date).Distinct().OrderBy(x => x))
            {
                var balance = GetDailyBalanceMinutes(wd, employeeId);

                meta[wd].Extra = balance.OverMinutes / 60.0;
                meta[wd].CanStart = CanTrimStart(wd, employeeId);
                meta[wd].CanEnd = CanTrimEnd(wd, employeeId);
                meta[wd].Locked = IsLockedEdgeDay(wd, employeeId);
            }
        }

        private sealed class RollbackResult
        {
            public int RestoredMinutes { get; init; }
            public HashSet<DateTime> TouchedDays { get; init; } = new();
            public Dictionary<DateTime, int> MinutesByDay { get; init; } = new();
        }

        private static (int IsoYear, int IsoWeek) GetIsoWeekKey(DateTime d)
             => (System.Globalization.ISOWeek.GetYear(d), System.Globalization.ISOWeek.GetWeekOfYear(d));

        private static List<DateTime> GetMonthWorkdays(int year, int month)
        {
            var first = new DateTime(year, month, 1);
            var last = new DateTime(year, month, DateTime.DaysInMonth(year, month));

            return Enumerable.Range(0, (last - first).Days + 1)
                .Select(i => first.AddDays(i).Date)
                .Where(IsWorkday)
                .ToList();
        }

        private static bool IsPartialMonthWeek((int IsoYear, int IsoWeek) key, int month)
        {
            var monday = FirstDayOfIsoWeek(key.IsoYear, key.IsoWeek);

            var weekWorkdays = Enumerable.Range(0, 7)
                .Select(i => monday.AddDays(i).Date)
                .Where(IsWorkday)
                .ToList();

            return weekWorkdays.Any(d => d.Month == month) &&
                   weekWorkdays.Any(d => d.Month != month);
        }

        private async Task BalanceMonthPartialScopesAsync(int year, int month, int employeeId = DefaultEmployeeId)
        {
            var monthDays = GetMonthWorkdays(year, month);
            if (monthDays.Count == 0)
                return;

            var orderedMonthGroups = monthDays
                .GroupBy(GetIsoWeekKey)
                .Select(g => g.OrderBy(d => d).ToList())
                .Where(g => g.Count > 0)
                .OrderBy(g => g.First())
                .ToList();

            var partialGroups = orderedMonthGroups
                .Where(g => IsPartialMonthWeek(GetIsoWeekKey(g[0]), month))
                .ToList();

            if (partialGroups.Count == 0)
            {
                return;
            }

            List<DateTime>? balanceScope;
            if (partialGroups.Count >= 2)
            {
                balanceScope = partialGroups
                    .SelectMany(g => g)
                    .Select(d => d.Date)
                    .Distinct()
                    .OrderBy(d => d)
                    .ToList();
            }
            else
            {
                balanceScope = BuildSinglePartialFallbackScope(orderedMonthGroups, partialGroups[0], month);

                if (balanceScope == null || balanceScope.Count < 2)
                {
                    return;
                }
            }

            if (!CanStrictlyBalancePartialPool(balanceScope, employeeId, out var reason))
            {              
                return;
            }

            await BalanceOneWeekAsync(balanceScope, employeeId);

            var totals = GetScopeTotals(balanceScope, employeeId);
        }

        private double RestoreUnderworkSmart(DateTime day, double hours, int employeeId)
        {
            const double EPS = 1e-6;

            if (hours <= EPS)
                return 0;

            bool canStart = CanExtendStart(day, employeeId);
            bool canEnd = CanExtendEnd(day, employeeId);

            if (!canStart && !canEnd)
                return 0;

            int startCapMin = GetExtendStartCapacityMinutesByWindow(day, employeeId);
            int endCapMin = GetExtendEndCapacityMinutesByWindow(day, employeeId);

            bool preferStart =
                canStart &&
                (!canEnd || startCapMin >= endCapMin);

            double placed = 0.0;

            if (preferStart)
            {
                if (canStart)
                    placed += ExtendStartAuto(day, hours - placed, employeeId);

                if (hours - placed > EPS && canEnd)
                    placed += ExtendEndAuto(day, hours - placed, employeeId);
            }
            else
            {
                if (canEnd)
                    placed += ExtendEndAuto(day, hours - placed, employeeId);

                if (hours - placed > EPS && canStart)
                    placed += ExtendStartAuto(day, hours - placed, employeeId);
            }

            return placed;
        }

        private bool DayHadAutoGeneratedEvents(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            using var db = new AppDbContext();

            return db.Events.Any(e =>
                e.EmployeeId == employeeId &&
                !e.IsDeleted &&
                e.IsAutoGenerated &&
                e.StartTime >= dayStart &&
                e.StartTime < dayEnd);
        }

        private void RebuildDayAfterChange(DateTime day, int employeeId, bool dayHadAuto)
        {
            var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

            if (dayHadAuto)
            {
                SettingsService.DeleteComputedDaySettingsForDate(day, employeeId);

                gen.RegenerateDailyEventsAsync(day, preserveUserSettings: false, ensureLunchAfterGeneration: false)
                   .GetAwaiter()
                   .GetResult();
            }
            else
            {
                RemoveAutoGeneratedEvents(employeeId, day);
                AdjustDaySettingsAfterChange(day, employeeId);

                gen.RegenerateDailyEventsAsync(day, preserveUserSettings: false, ensureLunchAfterGeneration: false)
                   .GetAwaiter()
                   .GetResult();
            }

            EnsureLunchInsideWorkWindowAsync(employeeId, day)
                .GetAwaiter()
                .GetResult();

            ReconcileSelfTrimPoolForDay(day, employeeId);
        }

        private int GetEdgeAutoTrimCapacityMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            return GetTrimStartCapacityMinutes(day, employeeId)
                 + GetTrimEndCapacityMinutes(day, employeeId);
        }

        private static bool LeavesTinyBalanceEdgeBlock(int capacityMin, int takeMin)
        {
            int remainder = RoundDownToQuantum(capacityMin - takeMin);
            return remainder > 0 && remainder < MIN_BALANCE_EDGE_BLOCK_MIN;
        }

        private bool NeedsExternalBalanceTransfer(DateTime day, int extraMin, int employeeId = DefaultEmployeeId)
        {
            extraMin = RoundDownToQuantum(extraMin);
            if (extraMin < QUANTUM_MIN)
                return false;

            if (IsLockedEdgeDay(day, employeeId))
                return true;

            return GetEdgeAutoTrimCapacityMinutes(day, employeeId) < extraMin;
        }

        private (double extra, double under) GetScopeTotals(IEnumerable<DateTime> days, int employeeId = DefaultEmployeeId)
        {
            var scope = days
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            return (
                extra: scope.Sum(d => GetBalanceOverMinutes(d, employeeId)) / 60.0,
                under: scope.Sum(d => GetBalanceUnderMinutes(d, employeeId)) / 60.0
            );
        }

        private bool CanStrictlyBalancePartialPool(
            IReadOnlyList<DateTime> scopeDays,
            int employeeId,
            out string reason)
        {
            int lockedNeedMin = 0;
            int donorSpareMin = 0;

            foreach (var day in scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d))
            {
                int overMin = GetBalanceOverMinutes(day, employeeId);
                int edgeCapMin = GetEdgeAutoTrimCapacityMinutes(day, employeeId);

                if (overMin <= 0)
                {
                    donorSpareMin += edgeCapMin;
                    continue;
                }

                bool isLocked = IsLockedEdgeDay(day, employeeId);
                int selfTrimMin = isLocked ? 0 : Math.Min(edgeCapMin, overMin);
                int externalNeedMin = overMin - selfTrimMin;

                if (externalNeedMin >= QUANTUM_MIN)
                    lockedNeedMin += externalNeedMin;

                donorSpareMin += Math.Max(0, edgeCapMin - overMin);
            }

            if (donorSpareMin < lockedNeedMin)
            {
                reason = $"lockedNeedMin={lockedNeedMin}, donorSpareMin={donorSpareMin}";
                return false;
            }

            reason = "ok";
            return true;
        }

        private List<DateTime>? BuildSinglePartialFallbackScope(
            List<List<DateTime>> orderedMonthGroups,
            List<DateTime> partialGroup,
            int month)
        {
            if (orderedMonthGroups.Count == 0 || partialGroup.Count == 0)
                return null;

            var partialKey = GetIsoWeekKey(partialGroup[0]);
            var firstKey = GetIsoWeekKey(orderedMonthGroups.First()[0]);
            var lastKey = GetIsoWeekKey(orderedMonthGroups.Last()[0]);

            List<DateTime>? companionFullGroup = null;

            if (partialKey == firstKey)
            {
                companionFullGroup = orderedMonthGroups
                    .Where(g => g.Count > 0)
                    .Where(g => GetIsoWeekKey(g[0]) != partialKey)
                    .FirstOrDefault(g => !IsPartialMonthWeek(GetIsoWeekKey(g[0]), month));
            }
            else if (partialKey == lastKey)
            {
                companionFullGroup = orderedMonthGroups
                    .Where(g => g.Count > 0)
                    .Where(g => GetIsoWeekKey(g[0]) != partialKey)
                    .Reverse()
                    .FirstOrDefault(g => !IsPartialMonthWeek(GetIsoWeekKey(g[0]), month));
            }
            else
            {
                companionFullGroup = orderedMonthGroups
                    .Where(g => g.Count > 0)
                    .Where(g => GetIsoWeekKey(g[0]) != partialKey)
                    .FirstOrDefault(g => !IsPartialMonthWeek(GetIsoWeekKey(g[0]), month));
            }

            if (companionFullGroup == null || companionFullGroup.Count == 0)
                return null;

            return partialGroup
                .Concat(companionFullGroup)
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();
        }

        private sealed class DayBudget
        {
            public DateTime Day { get; init; }
            public int Minutes { get; set; }
        }

        public sealed class PdfDayCompensation
        {
            public int ExtraOffsetMinutes { get; set; }
            public int UnderOffsetMinutes { get; set; }
        }

        private void ApplyPdfCompensationForScope(
            IReadOnlyList<DateTime> scopeDays,
            Dictionary<DateTime, PdfDayCompensation> result,
            int employeeId = DefaultEmployeeId)
        {
            var extras = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .Select(d => new DayBudget
                {
                    Day = d,
                    Minutes = GetBalanceOverMinutes(d, employeeId)
                })
                .Where(x => x.Minutes >= QUANTUM_MIN)
                .ToList();

            var unders = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .Select(d => new DayBudget
                {
                    Day = d,
                    Minutes = GetBalanceUnderMinutes(d, employeeId)
                })
                .Where(x => x.Minutes >= QUANTUM_MIN)
                .ToList();

            int i = 0;
            int j = 0;

            while (i < extras.Count && j < unders.Count)
            {
                int move = Math.Min(extras[i].Minutes, unders[j].Minutes);
                move = RoundDownToQuantum(move);

                if (move < QUANTUM_MIN)
                    break;

                if (result.TryGetValue(extras[i].Day, out var extraComp))
                    extraComp.ExtraOffsetMinutes += move;

                if (result.TryGetValue(unders[j].Day, out var underComp))
                    underComp.UnderOffsetMinutes += move;

                extras[i].Minutes -= move;
                unders[j].Minutes -= move;

                if (extras[i].Minutes < QUANTUM_MIN)
                    i++;

                if (unders[j].Minutes < QUANTUM_MIN)
                    j++;
            }
        }

        public Dictionary<DateTime, PdfDayCompensation> BuildWeeklyPdfCompensation(
            DateTime anyDate,
            int employeeId = DefaultEmployeeId)
        {
            var weekDays = GetWeekWorkdays(anyDate)
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            using var _settingsCache = SettingsService.BeginReadCache(employeeId, weekDays);
            using var _cache = BeginEventReadCache(employeeId, weekDays);

            var result = weekDays.ToDictionary(
                d => d,
                _ => new PdfDayCompensation());

            ApplyPdfCompensationForScope(weekDays, result, employeeId);
            return result;
        }

        public Dictionary<DateTime, PdfDayCompensation> BuildMonthPdfCompensation(
    int year,
    int month,
    int employeeId = DefaultEmployeeId)
        {
            var monthDays = GetMonthWorkdays(year, month)
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            using var _settingsCache = SettingsService.BeginReadCache(employeeId, monthDays);
            using var _cache = BeginEventReadCache(employeeId, monthDays);

            var result = monthDays.ToDictionary(
                d => d,
                _ => new PdfDayCompensation());

            if (monthDays.Count == 0)
                return result;

            var orderedMonthGroups = monthDays
                .GroupBy(GetIsoWeekKey)
                .Select(g => g.OrderBy(d => d).ToList())
                .Where(g => g.Count > 0)
                .OrderBy(g => g.First())
                .ToList();

            var partialGroups = orderedMonthGroups
                .Where(g => IsPartialMonthWeek(GetIsoWeekKey(g[0]), month))
                .ToList();

            var processed = new HashSet<DateTime>();

            void ApplyScope(List<DateTime> scope)
            {
                ApplyPdfCompensationForScope(scope, result, employeeId);

                foreach (var d in scope.Select(x => x.Date))
                {
                    if (d.Month == month && d.Year == year)
                        processed.Add(d);
                }
            }

            if (partialGroups.Count >= 2)
            {
                var partialScope = partialGroups
                    .SelectMany(g => g)
                    .Select(d => d.Date)
                    .Distinct()
                    .OrderBy(d => d)
                    .ToList();

                ApplyScope(partialScope);
            }
            else if (partialGroups.Count == 1)
            {
                var fallbackScope = BuildSinglePartialFallbackScope(
                    orderedMonthGroups,
                    partialGroups[0],
                    month);

                if (fallbackScope != null && fallbackScope.Count >= 2)
                    ApplyScope(fallbackScope);
                else
                    ApplyScope(partialGroups[0]);
            }

            foreach (var group in orderedMonthGroups)
            {
                if (group.Any(d => processed.Contains(d.Date)))
                    continue;

                ApplyScope(group);
            }

            return result;
        }

        private bool CanExtendStart(DateTime day, int employeeId = DefaultEmployeeId)
            => GetExtendStartCapacityMinutesByWindow(day, employeeId) >= QUANTUM_MIN;

        private bool CanExtendEnd(DateTime day, int employeeId = DefaultEmployeeId)
            => GetExtendEndCapacityMinutesByWindow(day, employeeId) >= QUANTUM_MIN;

        private sealed class SelfTrimEntry
        {
            public DateTime Day { get; set; }
            public TransferEdge Edge { get; set; }
            public int Minutes { get; set; }
        }

        private static class SelfTrimStore
        {
            public static void Clamp(int employeeId, DateTime day, TransferEdge edge, int maxMinutes)
            {
                day = day.Date;
                maxMinutes = Math.Max(0, RoundDownToQuantum(maxMinutes));

                using var db = new AppDbContext();

                var row = db.BalanceSelfTrims.SingleOrDefault(x =>
                    x.EmployeeId == employeeId &&
                    x.Day == day &&
                    x.Edge == (int)edge);

                if (row == null)
                    return;

                if (maxMinutes < QUANTUM_MIN)
                {
                    db.BalanceSelfTrims.Remove(row);
                    db.SaveChanges();
                    return;
                }

                if (row.Minutes > maxMinutes)
                {
                    row.Minutes = maxMinutes;
                    row.UpdatedAtUtc = DateTime.UtcNow;
                    db.SaveChanges();
                }
            }

            public static void Add(int employeeId, DateTime day, TransferEdge edge, int minutes)
            {
                minutes = minutes - minutes % QUANTUM_MIN;
                if (minutes < QUANTUM_MIN)
                    return;

                day = day.Date;
                var now = DateTime.UtcNow;

                using var db = new AppDbContext();

                var row = db.BalanceSelfTrims.SingleOrDefault(x =>
                    x.EmployeeId == employeeId &&
                    x.Day == day &&
                    x.Edge == (int)edge);

                if (row == null)
                {
                    db.BalanceSelfTrims.Add(new BalanceSelfTrim
                    {
                        EmployeeId = employeeId,
                        Day = day,
                        Edge = (int)edge,
                        Minutes = minutes,
                        CreatedAtUtc = now,
                        UpdatedAtUtc = now
                    });
                }
                else
                {
                    row.Minutes += minutes;
                    row.UpdatedAtUtc = now;
                }

                db.SaveChanges();
            }

            public static int GetTotal(int employeeId, DateTime day)
            {
                day = day.Date;

                using var db = new AppDbContext();
                return db.BalanceSelfTrims
                    .Where(x => x.EmployeeId == employeeId && x.Day == day)
                    .Select(x => (int?)x.Minutes)
                    .Sum() ?? 0;
            }

            public static List<SelfTrimEntry> GetSnapshot(int employeeId, DateTime day)
            {
                day = day.Date;

                using var db = new AppDbContext();
                return db.BalanceSelfTrims
                    .AsNoTracking()
                    .Where(x => x.EmployeeId == employeeId && x.Day == day)
                    .OrderByDescending(x => x.Minutes)
                    .ThenBy(x => x.Edge)
                    .Select(x => new SelfTrimEntry
                    {
                        Day = x.Day,
                        Edge = (TransferEdge)x.Edge,
                        Minutes = x.Minutes
                    })
                    .ToList();
            }

            public static List<SelfTrimEntry> GetScopeSnapshot(int employeeId, IEnumerable<DateTime> days)
            {
                var dayList = days
                    .Select(x => x.Date)
                    .Distinct()
                    .ToList();

                if (dayList.Count == 0)
                    return new List<SelfTrimEntry>();

                using var db = new AppDbContext();
                return db.BalanceSelfTrims
                    .AsNoTracking()
                    .Where(x => x.EmployeeId == employeeId && dayList.Contains(x.Day))
                    .OrderBy(x => x.Day)
                    .ThenBy(x => x.Edge)
                    .Select(x => new SelfTrimEntry
                    {
                        Day = x.Day,
                        Edge = (TransferEdge)x.Edge,
                        Minutes = x.Minutes
                    })
                    .ToList();
            }

            public static void Consume(int employeeId, DateTime day, TransferEdge edge, int minutes)
            {
                minutes = minutes - minutes % QUANTUM_MIN;
                if (minutes < QUANTUM_MIN)
                    return;

                day = day.Date;

                using var db = new AppDbContext();
                var row = db.BalanceSelfTrims.SingleOrDefault(x =>
                    x.EmployeeId == employeeId &&
                    x.Day == day &&
                    x.Edge == (int)edge);

                if (row == null)
                    return;

                row.Minutes -= minutes;

                if (row.Minutes < QUANTUM_MIN)
                    db.BalanceSelfTrims.Remove(row);
                else
                    row.UpdatedAtUtc = DateTime.UtcNow;

                db.SaveChanges();
            }

            public static void ClearDates(int employeeId, IEnumerable<DateTime> dates)
            {
                var dayList = dates
                    .Select(x => x.Date)
                    .Distinct()
                    .ToList();

                if (dayList.Count == 0)
                    return;

                using var db = new AppDbContext();
                var rows = db.BalanceSelfTrims
                    .Where(x => x.EmployeeId == employeeId && dayList.Contains(x.Day))
                    .ToList();

                if (rows.Count == 0)
                    return;

                db.BalanceSelfTrims.RemoveRange(rows);
                db.SaveChanges();
            }
        }

        private async Task<PoolRestoreResult> RollbackTransferPoolForScopeAsync(
            IReadOnlyList<DateTime> scopeDays,
            int employeeId = DefaultEmployeeId)
        {
            var days = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            int scopeUnderMin = days.Sum(d => GetBalanceUnderMinutes(d, employeeId));

            scopeUnderMin = RoundDownToQuantum(scopeUnderMin);
            if (scopeUnderMin < QUANTUM_MIN)
                return new PoolRestoreResult();

            var incoming = days
                .SelectMany(d => WorkTransferReportingService.GetTransfersTo(d, employeeId))
                .OrderByDescending(x => x.Minutes)
                .ThenBy(x => x.ToDay)
                .ThenBy(x => x.FromDay)
                .ToList();

            if (incoming.Count == 0)
                return new PoolRestoreResult();

            int left = scopeUnderMin;
            int restored = 0;
            var touched = new HashSet<DateTime>();
            var restoredByDay = new Dictionary<DateTime, int>();

            foreach (var entry in incoming)
            {
                if (left < QUANTUM_MIN)
                    break;

                int want = Math.Min(left, entry.Minutes);
                want = RoundDownToQuantum(want);

                if (want < QUANTUM_MIN)
                    continue;

                var putBackHours = RestoreTransferredMinutes(
                    entry.FromDay,
                    want,
                    entry.Edge,
                    employeeId);

                int putBackMin = RoundDownToQuantum((int)Math.Round(putBackHours * 60.0));
                if (putBackMin < QUANTUM_MIN)
                    continue;

                restored += putBackMin;
                left -= putBackMin;
                touched.Add(entry.FromDay.Date);
                AddRestoredMinutes(restoredByDay, entry.FromDay, putBackMin);

                WorkTransferReportingService.RemoveEntry(entry);

                int remainder = RoundDownToQuantum(entry.Minutes - putBackMin);
                if (remainder >= QUANTUM_MIN)
                {
                    WorkTransferReportingService.AddTransfer(
                        entry.FromDay.Date,
                        entry.ToDay.Date,
                        remainder,
                        entry.Edge,
                        employeeId);
                }
            }

            if (restored >= QUANTUM_MIN)
            {
                var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

                foreach (var d in touched.OrderBy(x => x))
                {
                    await gen.RegenerateDailyEventsAsync(d, preserveUserSettings: true, ensureLunchAfterGeneration: false);
                    await EnsureLunchInsideWorkWindowAsync(employeeId, d, callRegenerate: false);
                }

                NotifyAutoEventsGenerated();
            }

            return new PoolRestoreResult
            {
                RestoredMinutes = restored,
                TouchedDays = touched,
                MinutesByDay = restoredByDay
            };
        }

        private static readonly System.Threading.AsyncLocal<bool> _trackSelfTrim = new();

        private sealed class SelfTrimTrackingScope : IDisposable
        {
            private readonly bool _prev;

            public SelfTrimTrackingScope()
            {
                _prev = _trackSelfTrim.Value;
                _trackSelfTrim.Value = true;
            }

            public void Dispose()
            {
                _trackSelfTrim.Value = _prev;
            }
        }

        private static bool ShouldTrackSelfTrim()
            => _trackSelfTrim.Value;

        private sealed class PoolRestoreResult
        {
            public int RestoredMinutes { get; init; }
            public HashSet<DateTime> TouchedDays { get; init; } = new();
            public Dictionary<DateTime, int> MinutesByDay { get; init; } = new();
        }

        private static void AddRestoredMinutes(
            Dictionary<DateTime, int> bag,
            DateTime day,
            int minutes)
        {
            minutes = RoundDownToQuantum(minutes);
            if (minutes < QUANTUM_MIN)
                return;

            day = day.Date;

            if (bag.TryGetValue(day, out var current))
                bag[day] = current + minutes;
            else
                bag[day] = minutes;
        }

        private static void MergeRestoredMinutes(
            Dictionary<DateTime, int> target,
            IDictionary<DateTime, int> source)
        {
            foreach (var kv in source)
                AddRestoredMinutes(target, kv.Key, kv.Value);
        }

        private int GetExtendEndCapacityMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var resolved = GetResolvedWindow(day, employeeId);
            var daySoftEnd = day.Date.AddDays(1).AddMinutes(-QUANTUM_MIN);

            var nonLunch = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType != EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .ToList();

            var anchorStart = nonLunch.Any()
                ? nonLunch.Max(e => e.EndTime)
                : day + resolved.ArrivalTime;

            return Math.Max(0, RoundDownToQuantum((int)(daySoftEnd - anchorStart).TotalMinutes));
        }

        private int GetExtendStartCapacityMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var resolved = GetResolvedWindow(day, employeeId);

            var nonLunch = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType != EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .ToList();

            var anchorEnd = nonLunch.Any()
                ? nonLunch.Min(e => e.StartTime)
                : day + resolved.DepartureTime;

            return Math.Max(0, RoundDownToQuantum((int)(anchorEnd - day.Date).TotalMinutes));
        }

        private void ReconcileSelfTrimPoolForDay(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var startCap = GetExtendStartCapacityMinutesByWindow(day, employeeId);
            var endCap = GetExtendEndCapacityMinutesByWindow(day, employeeId);

            SelfTrimStore.Clamp(employeeId, day, TransferEdge.Start, startCap);
            SelfTrimStore.Clamp(employeeId, day, TransferEdge.End, endCap);
        }

        private static TimeSpan ParseDurationOrDefault(string? value, TimeSpan fallback)
        {
            return TimeSpan.TryParse(value, out var ts) && ts > TimeSpan.Zero
                ? ts
                : fallback;
        }

        private static bool IsSpecialForLunchWindow(Event e)
             => e.EventType != EventType.Lunch && !IsCreditedWorkTime(e);

        private (TimeSpan arr, TimeSpan dep) GetLunchPolicyWindowForDay(
            DateTime day,
            int employeeId = DefaultEmployeeId)
        {
            var resolved = GetResolvedWindow(day, employeeId);

            var arr = resolved.ArrivalTime;
            var dep = resolved.DepartureTime;

            var manualCredited = GetEventsForDay(employeeId, day)
                .Where(e =>
                    !e.IsDeleted &&
                    !e.IsAutoGenerated &&
                    IsCreditedWorkTime(e))
                .OrderBy(e => e.StartTime)
                .ToList();

            if (manualCredited.Count > 0)
            {
                var first = manualCredited.Min(e => e.StartTime.TimeOfDay);
                var last = manualCredited.Max(e => e.EndTime.TimeOfDay);

                if (first < arr) arr = first;
                if (last > dep) dep = last;
            }

            return (arr, dep);
        }

        private static int GetTargetLunchCount(TimeSpan gross)
        {
            if (gross > TimeSpan.FromHours(12))
                return 2;

            if (gross > TimeSpan.FromHours(4))
                return 1;

            return 0;
        }

        private static int GetManualWorkDrivenLunchCount(TimeSpan gross)
        {
            if (gross > TimeSpan.FromHours(12))
                return 2;

            if (gross > TimeSpan.FromHours(4))
                return 1;

            return 0;
        }

        private TimeSpan GetManualAdjustableWorkGross(DateTime day, int employeeId = DefaultEmployeeId)
        {
            return GetEffectiveCreditedWorkGross(
                day,
                GetEventsForDay(employeeId, day),
                e => !e.IsAutoGenerated && e.EventType.IsAutoAdjustableWork());
        }

        private static TimeSpan GetEffectiveCreditedWorkGross(
            DateTime day,
            IEnumerable<Event> dayEvents,
            Func<Event, bool> workSelector)
        {
            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            var specialBlockers = dayEvents
                .Where(e =>
                    !e.IsDeleted &&
                    e.EventType != EventType.Lunch &&
                    !IsCreditedWorkTime(e))
                .Select(e => (
                    s: e.StartTime < dayStart ? dayStart : e.StartTime,
                    e: e.EndTime > dayEnd ? dayEnd : e.EndTime))
                .Where(x => x.e > x.s)
                .ToList();

            var work = dayEvents
                .Where(e =>
                    !e.IsDeleted &&
                    e.EventType != EventType.Lunch &&
                    workSelector(e))
                .Select(e => (
                    s: e.StartTime < dayStart ? dayStart : e.StartTime,
                    e: e.EndTime > dayEnd ? dayEnd : e.EndTime))
                .Where(x => x.e > x.s)
                .ToList();

            var activeWork = SubtractIntervals(work, specialBlockers);

            return TimeSpan.FromTicks(
                MergeIntervals(activeWork)
                    .Sum(x => (x.e - x.s).Ticks));
        }

        private static int GetEventWindowDrivenLunchCount(DateTime day, IEnumerable<Event> dayEvents)
        {
            return GetManualWorkDrivenLunchCount(
                GetEffectiveCreditedWorkGross(
                    day,
                    dayEvents,
                    IsCreditedWorkTime));
        }

        private static List<(DateTime s, DateTime e)> SubtractIntervals(
            IEnumerable<(DateTime s, DateTime e)> source,
            IEnumerable<(DateTime s, DateTime e)> blockers)
        {
            var mergedBlockers = MergeIntervals(blockers);
            var result = new List<(DateTime s, DateTime e)>();

            foreach (var segment in source.Where(x => x.e > x.s).OrderBy(x => x.s))
            {
                var cursor = segment.s;

                foreach (var blocker in mergedBlockers)
                {
                    if (blocker.e <= cursor)
                        continue;

                    if (blocker.s >= segment.e)
                        break;

                    if (blocker.s > cursor)
                        result.Add((cursor, blocker.s < segment.e ? blocker.s : segment.e));

                    if (blocker.e > cursor)
                        cursor = blocker.e > segment.e ? segment.e : blocker.e;

                    if (cursor >= segment.e)
                        break;
                }

                if (cursor < segment.e)
                    result.Add((cursor, segment.e));
            }

            return MergeIntervals(result);
        }

        private static bool HasCreditedWorkOutsideSpecial(
            IEnumerable<Event> dayEvents,
            DateTime winS,
            DateTime winE,
            IEnumerable<(DateTime s, DateTime e)> specialIntervals)
        {
            var credited = dayEvents
                .Where(e => !e.IsDeleted && IsCreditedWorkTime(e))
                .Select(e => (
                    s: e.StartTime < winS ? winS : e.StartTime,
                    e: e.EndTime > winE ? winE : e.EndTime))
                .Where(x => x.e > x.s)
                .ToList();

            if (credited.Count == 0)
                return false;

            return SubtractIntervals(credited, specialIntervals)
                .Any(x => x.e - x.s >= TimeSpan.FromMinutes(QUANTUM_MIN));
        }

        private bool HasManualDayOverride(DateTime day, int employeeId = DefaultEmployeeId)
    => SettingsService.GetManualDaySettingsForDate(day, employeeId) != null;

        private List<Event> GetPinnedEventsForBalance(DateTime day, int employeeId = DefaultEmployeeId)
        {
            return GetEventsForDay(employeeId, day)
                .Where(e =>
                    !e.IsDeleted &&
                    e.EventType != EventType.Lunch &&
                    !IsBalanceAdjustableWork(e))
                .OrderBy(e => e.StartTime)
                .ToList();
        }

        private (TimeSpan ls, TimeSpan le) GetLunchInsideBalanceWindow(
            DateTime day,
            TimeSpan arr,
            TimeSpan dep,
            int employeeId = DefaultEmployeeId)
        {
            var manualLunch = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && !e.IsAutoGenerated && e.EventType == EventType.Lunch)
                .OrderBy(e => e.StartTime)
                .FirstOrDefault();

            if (manualLunch != null)
            {
                var ls0 = manualLunch.StartTime.TimeOfDay;
                var le0 = manualLunch.EndTime.TimeOfDay;

                if (le0 > ls0 && ls0 >= arr && le0 <= dep)
                    return (ls0, le0);

                return (TimeSpan.Zero, TimeSpan.Zero);
            }

            var resolved = GetResolvedWindow(day, employeeId);
            var ls = resolved.LunchStart;
            var le = resolved.LunchEnd;

            if (le > ls && ls >= arr && le <= dep)
                return (ls, le);

            return (TimeSpan.Zero, TimeSpan.Zero);
        }

        private int GetTrimStartCapacityMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            if (HasManualDayOverride(day, employeeId))
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);
            var arr = resolved.ArrivalTime;
            var dep = resolved.DepartureTime;

            var pinned = GetPinnedEventsForBalance(day, employeeId);
            var boundary = pinned.Count > 0
                ? pinned.Min(e => e.StartTime).TimeOfDay
                : dep;

            if (boundary <= arr)
                return 0;

            return Math.Max(0, RoundDownToQuantum((int)(boundary - arr).TotalMinutes));
        }

        private int GetTrimEndCapacityMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            if (HasManualDayOverride(day, employeeId))
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);
            var arr = resolved.ArrivalTime;
            var dep = resolved.DepartureTime;

            var pinned = GetPinnedEventsForBalance(day, employeeId);
            var boundary = pinned.Count > 0
                ? pinned.Max(e => e.EndTime).TimeOfDay
                : arr;

            if (dep <= boundary)
                return 0;

            return Math.Max(0, RoundDownToQuantum((int)(dep - boundary).TotalMinutes));
        }

        private int GetExtendStartCapacityMinutesByWindow(DateTime day, int employeeId = DefaultEmployeeId)
        {
            if (HasManualDayOverride(day, employeeId))
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);
            var baseResolved = GetBaseResolvedWindow(day, employeeId);

            if (resolved.ArrivalTime <= baseResolved.ArrivalTime)
                return 0;

            return Math.Max(0, RoundDownToQuantum((int)(resolved.ArrivalTime - baseResolved.ArrivalTime).TotalMinutes));
        }

        private int GetExtendEndCapacityMinutesByWindow(DateTime day, int employeeId = DefaultEmployeeId)
        {
            if (HasManualDayOverride(day, employeeId))
                return 0;

            var resolved = GetResolvedWindow(day, employeeId);
            var baseResolved = GetBaseResolvedWindow(day, employeeId);

            if (baseResolved.DepartureTime <= resolved.DepartureTime)
                return 0;

            return Math.Max(0, RoundDownToQuantum((int)(baseResolved.DepartureTime - resolved.DepartureTime).TotalMinutes));
        }

        private async Task RebuildTouchedDaysAsync(IEnumerable<DateTime> days, int employeeId)
        {
            var touched = days
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            if (touched.Count == 0)
                return;

            var gen = new AutomaticEventsGeneratorService(this, _ => Task.FromResult(false), employeeId);

            foreach (var d in touched)
            {
                await gen.RegenerateDailyEventsAsync(d, preserveUserSettings: true, ensureLunchAfterGeneration: false);
                await EnsureLunchInsideWorkWindowAsync(employeeId, d, callRegenerate: false);
            }

            NotifyAutoEventsGenerated();
        }

        private int TryTrimOneQuantum(
            DateTime day,
            int employeeId,
            Dictionary<DateTime, bool> preferEnd,
            out TransferEdge? usedEdge)
        {
            usedEdge = null;

            if (IsLockedEdgeDay(day, employeeId))
                return 0;

            var edgeOptions = new[]
                {
                    new
                    {
                        Edge = TransferEdge.End,
                        Capacity = GetTrimEndCapacityMinutes(day, employeeId),
                        Preferred = preferEnd[day]
                    },
                    new
                    {
                        Edge = TransferEdge.Start,
                        Capacity = GetTrimStartCapacityMinutes(day, employeeId),
                        Preferred = !preferEnd[day]
                    }
                }
                .Where(x => x.Capacity >= QUANTUM_MIN)
                .ToList();

            if (edgeOptions.Count == 0)
                return 0;

            var safeOptions = edgeOptions
                .Where(x => !LeavesTinyBalanceEdgeBlock(x.Capacity, QUANTUM_MIN))
                .ToList();

            var trimOrder = (safeOptions.Count > 0 ? safeOptions : edgeOptions)
                .OrderByDescending(x => x.Capacity)
                .ThenByDescending(x => x.Preferred)
                .Select(x => x.Edge)
                .ToList();

            int cutMin = 0;

            foreach (var edge in trimOrder)
            {
                var cut = edge == TransferEdge.End
                    ? TrimEndAuto(day, QUANTUM_MIN / 60.0, employeeId)
                    : TrimStartAuto(day, QUANTUM_MIN / 60.0, employeeId);

                cutMin = RoundDownToQuantum(ToWholeMinutes(cut));
                if (cutMin >= QUANTUM_MIN)
                {
                    usedEdge = edge;
                    break;
                }
            }

            if (cutMin >= QUANTUM_MIN)
                preferEnd[day] = !preferEnd[day];
            else
                usedEdge = null;

            return cutMin;
        }

        private sealed class PlannedEdgeTrim
        {
            public int TotalMinutes { get; set; }
            public int StartMinutes { get; set; }
            public int EndMinutes { get; set; }
        }

        private PlannedEdgeTrim ApplyPlannedTrim(
            DateTime day,
            int minutes,
            int employeeId,
            Dictionary<DateTime, bool> preferEnd)
        {
            int left = RoundDownToQuantum(minutes);
            var result = new PlannedEdgeTrim();

            while (left >= QUANTUM_MIN)
            {
                int step = TryTrimOneQuantum(day, employeeId, preferEnd, out var edge);
                if (step < QUANTUM_MIN || edge is null)
                    break;

                result.TotalMinutes += step;

                if (edge.Value == TransferEdge.Start)
                    result.StartMinutes += step;
                else
                    result.EndMinutes += step;

                left -= step;
            }

            return result;
        }

        private int TryRestoreOneQuantum(
            DateTime day,
            int employeeId,
            Dictionary<DateTime, bool> preferStart)
        {
            double placed = 0;

            if (preferStart[day] && CanExtendStart(day, employeeId))
            {
                placed = ExtendStartAuto(day, QUANTUM_MIN / 60.0, employeeId);
            }

            int placedMin = RoundDownToQuantum(ToWholeMinutes(placed));

            if (placedMin < QUANTUM_MIN && CanExtendEnd(day, employeeId))
            {
                placed = ExtendEndAuto(day, QUANTUM_MIN / 60.0, employeeId);
                placedMin = RoundDownToQuantum(ToWholeMinutes(placed));
            }

            if (placedMin < QUANTUM_MIN && CanExtendStart(day, employeeId))
            {
                placed = ExtendStartAuto(day, QUANTUM_MIN / 60.0, employeeId);
                placedMin = RoundDownToQuantum(ToWholeMinutes(placed));
            }

            if (placedMin >= QUANTUM_MIN)
                preferStart[day] = !preferStart[day];

            return placedMin;
        }

        private async Task<int> TrimWeekRoundRobinAsync(
            IReadOnlyList<DateTime> scopeDays,
            int targetMinutes,
            int employeeId = DefaultEmployeeId)
        {
            var days = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            int target = RoundDownToQuantum(targetMinutes);
            if (target < QUANTUM_MIN)
                return 0;

            var candidates = days
                .Where(d => !IsLockedEdgeDay(d, employeeId))
                .Where(d => HasBalanceAdjustableWork(d, employeeId))
                .Select(d =>
                {
                    int extraMin = GetBalanceOverMinutes(d, employeeId);
                    int capMin = GetEdgeAutoTrimCapacityMinutes(d, employeeId);

                    return (Day: d, CapacityMinutes: Math.Min(extraMin, capMin));
                })
                .Where(x => x.CapacityMinutes >= QUANTUM_MIN)
                .ToList();

            if (candidates.Count == 0)
                return 0;

            var plan = BuildEvenTrimPlan(candidates, target);
            if (plan.Count == 0)
                return 0;

            var touched = new HashSet<DateTime>();
            var preferEnd = days.ToDictionary(d => d, _ => true);

            int totalCut = 0;

            using var _track = new SelfTrimTrackingScope();

            foreach (var kv in plan.OrderBy(x => x.Key))
            {
                var applied = ApplyPlannedTrim(kv.Key, kv.Value, employeeId, preferEnd);

                if (applied.TotalMinutes >= QUANTUM_MIN)
                {
                    totalCut += applied.TotalMinutes;
                    touched.Add(kv.Key);
                }
            }

            await RebuildTouchedDaysAsync(touched, employeeId);
            return totalCut;
        }

        private async Task<int> RestoreWeekRoundRobinAsync(
            IReadOnlyList<DateTime> scopeDays,
            int maxRestoreMinutes,
            int employeeId = DefaultEmployeeId,
            ISet<DateTime>? skipDays = null)
        {
            var days = scopeDays
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            int left = RoundDownToQuantum(maxRestoreMinutes);
            if (left < QUANTUM_MIN)
                return 0;

            var skip = skipDays?.Select(d => d.Date).ToHashSet() ?? new HashSet<DateTime>();
            var touched = new HashSet<DateTime>();
            var preferStart = days.ToDictionary(d => d, _ => true);

            var remainingUnderByDay = days.ToDictionary(
                d => d,
                d => GetBalanceUnderMinutes(d, employeeId));

            var restoredByDay = days.ToDictionary(d => d, _ => 0);

            while (left >= QUANTUM_MIN)
            {
                var eligible = days
                    .Where(d =>
                        !skip.Contains(d) &&
                        remainingUnderByDay[d] >= QUANTUM_MIN)
                    .OrderBy(d => restoredByDay[d])
                    .ThenBy(d => d)
                    .ToList();

                if (eligible.Count == 0)
                    break;

                bool progressed = false;

                foreach (var day in eligible)
                {
                    if (left < QUANTUM_MIN)
                        break;

                    bool hasOutgoingTransfers = WorkTransferReportingService.GetTransfersFrom(day, employeeId).Any();
                    bool hasIncomingTransfers = WorkTransferReportingService.GetTransfersTo(day, employeeId).Any();

                    if (hasOutgoingTransfers && !hasIncomingTransfers)
                        continue;

                    int placedMin = TryRestoreOneQuantum(day, employeeId, preferStart);
                    if (placedMin < QUANTUM_MIN)
                        continue;

                    remainingUnderByDay[day] = Math.Max(
                        0,
                        RoundDownToQuantum(remainingUnderByDay[day] - placedMin));

                    restoredByDay[day] += placedMin;
                    left -= placedMin;
                    touched.Add(day);
                    progressed = true;
                }

                if (!progressed)
                    break;
            }

            await RebuildTouchedDaysAsync(touched, employeeId);
            return RoundDownToQuantum(maxRestoreMinutes) - left;
        }

        public void UpdateEventCollisionRaw(int eventId, bool hasCollision, int employeeId = DefaultEmployeeId)
        {
            using var db = new AppDbContext();

            var ev = db.Events.SingleOrDefault(e => e.Id == eventId && e.EmployeeId == employeeId);
            if (ev == null)
                return;

            if (ev.HasCollision == hasCollision)
                return;

            var affectedDates = EnumerateDatesInclusive(ev.StartTime, ev.EndTime);

            ev.HasCollision = hasCollision;
            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, affectedDates);
        }

        private static readonly AsyncLocal<EventReadCacheScope?> _eventReadCacheScope = new();

        private sealed class EventReadCacheScope : IDisposable
        {
            public EventReadCacheScope? Previous { get; }
            public int EmployeeId { get; }

            private readonly HashSet<DateTime> _days;
            private readonly Dictionary<DateTime, List<Event>> _eventsByDay = new();

            public EventReadCacheScope(EventReadCacheScope? previous, int employeeId, IEnumerable<DateTime> days)
            {
                Previous = previous;
                EmployeeId = employeeId;
                _days = days
                    .Select(d => d.Date)
                    .Distinct()
                    .ToHashSet();

                foreach (var kv in LoadEventsForDaysFromDatabase(employeeId, _days))
                    _eventsByDay[kv.Key] = kv.Value;

                _eventReadCacheScope.Value = this;
            }

            public bool Contains(DateTime day) => _days.Contains(day.Date);

            public bool TryGet(DateTime day, out List<Event> events)
                => _eventsByDay.TryGetValue(day.Date, out events!);

            public void Set(DateTime day, List<Event> events)
                => _eventsByDay[day.Date] = events;

            public void Invalidate(IEnumerable<DateTime> days)
            {
                foreach (var d in days.Select(x => x.Date).Distinct())
                    _eventsByDay.Remove(d);
            }

            public void Dispose()
            {
                _eventReadCacheScope.Value = Previous;
            }
        }

        public IDisposable BeginEventReadCache(int employeeId, IEnumerable<DateTime> days)
            => new EventReadCacheScope(_eventReadCacheScope.Value, employeeId, days);

        private EventReadCacheScope? GetEventReadCacheScopeForDay(int employeeId, DateTime day)
        {
            for (var scope = _eventReadCacheScope.Value; scope != null; scope = scope.Previous)
            {
                if (scope.EmployeeId == employeeId && scope.Contains(day))
                    return scope;
            }

            return null;
        }

        private void InvalidateEventCacheDays(int employeeId, IEnumerable<DateTime> days)
        {
            var normalized = days
                .Select(d => d.Date)
                .Distinct()
                .ToList();

            for (var scope = _eventReadCacheScope.Value; scope != null; scope = scope.Previous)
            {
                if (scope.EmployeeId == employeeId)
                    scope.Invalidate(normalized);
            }
        }

        private static List<DateTime> EnumerateDatesInclusive(DateTime start, DateTime end)
        {
            var s = start.Date;
            var e = end.Date;

            if (e < s)
                (s, e) = (e, s);

            return Enumerable.Range(0, (e - s).Days + 1)
                .Select(i => s.AddDays(i))
                .ToList();
        }

        private static List<DateTime> CollectAffectedDates(IEnumerable<Event> events)
        {
            return events
                .SelectMany(e => EnumerateDatesInclusive(e.StartTime, e.EndTime))
                .Distinct()
                .OrderBy(d => d)
                .ToList();
        }

        private static List<Event> LoadEventsForDayFromDatabase(int employeeId, DateTime date)
        {
            var dayStart = date.Date;
            var dayEnd = dayStart.AddDays(1);

            using var db = new AppDbContext();

            return db.Events
                .AsNoTracking()
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime >= dayStart &&
                            e.StartTime < dayEnd)
                .OrderBy(e => e.StartTime)
                .ToList();
        }

        private static Dictionary<DateTime, List<Event>> LoadEventsForDaysFromDatabase(int employeeId, IReadOnlyCollection<DateTime> days)
        {
            var normalizedDays = days
                .Select(d => d.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            var result = normalizedDays.ToDictionary(d => d, _ => new List<Event>());
            if (normalizedDays.Count == 0)
                return result;

            var rangeStart = normalizedDays[0];
            var rangeEndExclusive = normalizedDays[^1].AddDays(1);

            using var db = new AppDbContext();

            var rows = db.Events
                .AsNoTracking()
                .Where(e => e.EmployeeId == employeeId &&
                            !e.IsDeleted &&
                            e.StartTime >= rangeStart &&
                            e.StartTime < rangeEndExclusive)
                .OrderBy(e => e.StartTime)
                .ToList();

            foreach (var row in rows)
            {
                if (result.TryGetValue(row.StartTime.Date, out var bucket))
                    bucket.Add(row);
            }

            return result;
        }

        private List<(DateTime s, DateTime e)> GetManualLunchBlockers(
            DateTime day,
            TimeSpan arr,
            TimeSpan dep,
            int employeeId = DefaultEmployeeId)
        {
            var winS = day + arr;
            var winE = day + dep;

            return GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted &&
                            !e.IsAutoGenerated &&
                            e.EventType != EventType.Lunch &&
                            !e.EventType.IsAutoAdjustableWork())
                .Select(e => (
                    s: e.StartTime < winS ? winS : e.StartTime,
                    e: e.EndTime > winE ? winE : e.EndTime))
                .Where(x => x.e > x.s)
                .OrderBy(x => x.s)
                .ToList();
        }

        private int GetStableTargetLunchCount(
            DateTime day,
            TimeSpan arr,
            TimeSpan dep,
            int employeeId = DefaultEmployeeId)
        {
            var manualOverride = SettingsService.GetManualDaySettingsForDate(day, employeeId);
            var dayEvents = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted)
                .ToList();

            bool hasCreditedWorkTime = dayEvents.Any(IsCreditedWorkTime);

            if (manualOverride != null &&
                manualOverride.LunchStart == manualOverride.LunchEnd &&
                !hasCreditedWorkTime)
                return 0;

            int manualWorkTarget = GetManualWorkDrivenLunchCount(GetManualAdjustableWorkGross(day, employeeId));
            int eventWindowTarget = GetEventWindowDrivenLunchCount(day, dayEvents);
            return Math.Max(manualWorkTarget, eventWindowTarget);
        }

        private void MergeAdjacentAutoWorkBlocks(DateTime day, int employeeId = DefaultEmployeeId)
        {
            var dayStart = day.Date;
            var dayEnd = dayStart.AddDays(1);

            using var db = new AppDbContext();

            var blocks = db.Events
                .Where(e =>
                    e.EmployeeId == employeeId &&
                    !e.IsDeleted &&
                    e.IsAutoGenerated &&
                    e.EventType == EventType.Work &&
                    e.StartTime >= dayStart &&
                    e.StartTime < dayEnd)
                .OrderBy(e => e.StartTime)
                .ToList();

            if (blocks.Count < 2)
                return;

            for (int i = 0; i < blocks.Count - 1;)
            {
                var a = blocks[i];
                var b = blocks[i + 1];

                if (a.EndTime >= b.StartTime)
                {
                    a.EndTime = a.EndTime >= b.EndTime ? a.EndTime : b.EndTime;

                    if (string.IsNullOrWhiteSpace(a.Title) && !string.IsNullOrWhiteSpace(b.Title))
                        a.Title = b.Title;

                    db.Events.Remove(b);
                    blocks.RemoveAt(i + 1);
                    continue;
                }

                i++;
            }

            db.SaveChanges();

            InvalidateEventCacheDays(employeeId, new[] { dayStart });
            InvalidateMonthsForDates(new[] { dayStart });
            InvalidateWeeksForDates(new[] { dayStart }, employeeId);
        }

        private sealed class DayTrimCapacity
        {
            public DateTime Day { get; init; }
            public int CapacityMinutes { get; set; }
        }

        private static Dictionary<DateTime, int> BuildEvenTrimPlan(
            IReadOnlyList<(DateTime Day, int CapacityMinutes)> source,
            int targetMinutes)
        {
            int left = RoundDownToQuantum(targetMinutes);

            var donors = source
                .Select(x => new DayTrimCapacity
                {
                    Day = x.Day.Date,
                    CapacityMinutes = RoundDownToQuantum(x.CapacityMinutes)
                })
                .Where(x => x.CapacityMinutes >= QUANTUM_MIN)
                .OrderByDescending(x => x.CapacityMinutes)
                .ThenBy(x => x.Day)
                .ToList();

            var plan = donors.ToDictionary(x => x.Day, _ => 0);

            while (left >= QUANTUM_MIN && donors.Count > 0)
            {
                bool allocatedGroup = false;

                for (int k = donors.Count; k >= 1; k--)
                {
                    int sameCut = RoundDownToQuantum(left / k);
                    if (sameCut < QUANTUM_MIN)
                        continue;

                    var group = donors.Take(k).ToList();

                    if (group.Any(x => x.CapacityMinutes < sameCut))
                        continue;

                    foreach (var donor in group)
                    {
                        donor.CapacityMinutes -= sameCut;
                        plan[donor.Day] += sameCut;
                    }

                    left -= sameCut * k;

                    donors = donors
                        .Where(x => x.CapacityMinutes >= QUANTUM_MIN)
                        .OrderByDescending(x => x.CapacityMinutes)
                        .ThenBy(x => x.Day)
                        .ToList();

                    allocatedGroup = true;
                    break;
                }

                if (!allocatedGroup)
                {
                    var top = donors[0];
                    top.CapacityMinutes -= QUANTUM_MIN;
                    plan[top.Day] += QUANTUM_MIN;
                    left -= QUANTUM_MIN;

                    donors = donors
                        .Where(x => x.CapacityMinutes >= QUANTUM_MIN)
                        .OrderByDescending(x => x.CapacityMinutes)
                        .ThenBy(x => x.Day)
                        .ToList();
                }
            }

            return plan
                .Where(x => x.Value >= QUANTUM_MIN)
                .ToDictionary(x => x.Key, x => x.Value);
        }

        private static bool IsSpecialForLunchSuppression(Event e)
            => !e.IsDeleted && e.EventType != EventType.Lunch && !IsCreditedWorkTime(e);

        private bool IsFullSpecialDayWithoutLunch(
            DateTime day,
            TimeSpan arr,
            TimeSpan dep,
            int targetLunchCount,
            TimeSpan lunchLen,
            int employeeId = DefaultEmployeeId)
        {
            if (dep <= arr)
                return false;

            var winS = day + arr;
            var winE = day + dep;

            var dayEvents = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted)
                .ToList();

            var specialIntervals = dayEvents
                .Where(IsSpecialForLunchSuppression)
                .Select(e => (
                    s: e.StartTime < winS ? winS : e.StartTime,
                    e: e.EndTime > winE ? winE : e.EndTime))
                .Where(x => x.e > x.s)
                .ToList();

            if (specialIntervals.Count == 0)
                return false;

            var merged = MergeIntervals(specialIntervals);

            if (HasCreditedWorkOutsideSpecial(dayEvents, winS, winE, merged))
                return false;

            var covered = TimeSpan.FromMinutes(
                merged.Sum(x => (x.e - x.s).TotalMinutes));

            var gross = winE - winS;

            var allowedLunch =
                TimeSpan.FromTicks(lunchLen.Ticks * Math.Max(1, targetLunchCount));

            var requiredCoverage = gross - allowedLunch;
            if (requiredCoverage < TimeSpan.Zero)
                requiredCoverage = TimeSpan.Zero;

            return covered >= requiredCoverage;
        }

        private sealed record DailyBalanceMinutes(
            int CreditedMinutes,
            int ExpectedMinutes,
            int OverMinutes,
            int UnderMinutes);

        private static bool IsPaidAbsenceForBalance(Event e)
            => e.EventType is EventType.DayOff
                or EventType.Illness
                or EventType.Vacation
                or EventType.Ocr
                or EventType.Doctor
                or EventType.Holiday;

        private static bool IsCreditedForBalance(Event e)
        {
            if (e.IsDeleted || e.EventType == EventType.Lunch)
                return false;

            return IsCreditedWorkTime(e) || IsPaidAbsenceForBalance(e);
        }

        private static (DateTime s, DateTime e)? ClipToDayForBalance(DateTime day, DateTime s, DateTime e)
        {
            var ds = day.Date;
            var de = ds.AddDays(1);

            var cs = s < ds ? ds : s;
            var ce = e > de ? de : e;

            return ce > cs ? (cs, ce) : null;
        }

        private DailyBalanceMinutes GetDailyBalanceMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            day = day.Date;

            if (!IsWorkday(day))
                return new DailyBalanceMinutes(0, 0, 0, 0);

            const int expectedMin = 8 * 60;

            var events = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && e.EventType != EventType.Lunch)
                .ToList();

            var paidAbsenceIntervals = events
                .Where(IsPaidAbsenceForBalance)
                .Select(e => ClipToDayForBalance(day, e.StartTime, e.EndTime))
                .Where(x => x.HasValue)
                .Select(x => x!.Value)
                .ToList();

            var mergedPaidAbsence = MergeIntervals(paidAbsenceIntervals);

            int paidAbsenceCreditMin = 0;

            foreach (var iv in mergedPaidAbsence)
            {
                if (paidAbsenceCreditMin >= expectedMin)
                    break;

                int minutes = Math.Max(0, (int)Math.Round((iv.e - iv.s).TotalMinutes));
                int take = Math.Min(minutes, expectedMin - paidAbsenceCreditMin);

                if (take <= 0)
                    continue;

                paidAbsenceCreditMin += take;
            }

            if (paidAbsenceCreditMin >= expectedMin)
            {
                return new DailyBalanceMinutes(
                    expectedMin,
                    expectedMin,
                    0,
                    0);
            }

            var workIntervals = events
                .Where(IsCreditedWorkTime)
                .Select(e => ClipToDayForBalance(day, e.StartTime, e.EndTime))
                .Where(x => x.HasValue)
                .Select(x => x!.Value)
                .ToList();

            var workOutsideAbsence = SubtractIntervals(workIntervals, mergedPaidAbsence);

            int workMin = RoundDownToQuantum(
                MergeIntervals(workOutsideAbsence)
                    .Sum(x => (int)(x.e - x.s).TotalMinutes));

            int creditedMin = RoundDownToQuantum(paidAbsenceCreditMin + workMin);

            int overMin = Math.Max(0, creditedMin - expectedMin);
            int underMin = Math.Max(0, expectedMin - creditedMin);

            return new DailyBalanceMinutes(
                creditedMin,
                expectedMin,
                RoundDownToQuantum(overMin),
                RoundDownToQuantum(underMin));
        }

        private int GetBalanceOverMinutes(DateTime day, int employeeId = DefaultEmployeeId)
            => GetDailyBalanceMinutes(day, employeeId).OverMinutes;

        private int GetBalanceUnderMinutes(DateTime day, int employeeId = DefaultEmployeeId)
            => GetDailyBalanceMinutes(day, employeeId).UnderMinutes;

        private int GetBalanceWorkedMinutes(DateTime day, int employeeId = DefaultEmployeeId)
            => GetDailyBalanceMinutes(day, employeeId).CreditedMinutes;
    }
}
