...
namespace TeacherScheduleApp.Services
{
    public class EventService
    {
        ...
        public static bool IsCreditedWorkTime(Event e)
            => e.EventType.IsCreditedWorkTime();
    ...
    int employeeId = DefaultEmployeeId)
            ...
            int employeeId = DefaultEmployeeId)
        {
            ...
            var manualCredited = GetEventsForDay(employeeId, day)
                .Where(e => !e.IsDeleted && !e.IsAutoGenerated && (IsCreditedWorkTime(e) || IsSpecialForLunchWindow(e)))
                .OrderBy(e => e.StartTime)
                .ToList();
        ...
        private TimeSpan GetManualAdjustableWorkGross(DateTime day, int employeeId = DefaultEmployeeId)
        {
            ...
            return TimeSpan.FromTicks(merged.Sum(x => (x.e - x.s).Ticks));
        }
        ...
        private static int GetEventWindowDrivenLunchCount(DateTime day, IEnumerable<Event> dayEvents)
        {
            ...
            var credited = dayEvents
                .Where(e =>
                    !e.IsDeleted &&
                    e.EventType != EventType.Lunch &&
                    (IsCreditedWorkTime(e) || IsSpecialForLunchWindow(e)))
                .Select(e => (
                    s: e.StartTime < dayStart ? dayStart : e.StartTime,
                    e: e.EndTime > dayEnd ? dayEnd : e.EndTime))
                .Where(x => x.e > x.s)
    ...
    => SettingsService.GetManualDaySettingsForDate(day, employeeId) != null;
        ...
        private DailyBalanceMinutes GetDailyBalanceMinutes(DateTime day, int employeeId = DefaultEmployeeId)
        {
            ...
            var events = GetEventsForDay(employeeId, day)
                .Where(IsCreditedForBalance)
                .ToList();

            var resolved = SettingsService.GetResolvedDaySettingsIgnoringComputed(day, employeeId);

            var normStart = day + resolved.ArrivalTime;
            var normEnd = day + resolved.DepartureTime;

            var lunch = resolved.LunchEnd > resolved.LunchStart
                ? new[] { (s: day + resolved.LunchStart, e: day + resolved.LunchEnd) }
            ...
            {
                if (ev.AllDay && normEnd > normStart)
                {
                    intervals.AddRange(SubtractIntervals(new[] { (normStart, normEnd) }, lunch));
                    continue;
                }

                var clipped = ClipToDayForBalance(day, ev.StartTime, ev.EndTime);
                if (!clipped.HasValue)
                    continue;

                var iv = clipped.Value;

                if (IsPaidAbsenceForBalance(ev) &&
                    (iv.e - iv.s) > TimeSpan.FromHours(8) &&
                    normEnd > normStart)
                {
                    intervals.AddRange(SubtractIntervals(new[] { (normStart, normEnd) }, lunch));
                }
            ...
            int creditedMin = RoundDownToQuantum(
