...
namespace TeacherScheduleApp.Services
{
    ...
    public sealed class ScheduleChangeProcessor
    {
            ...
            bool expandToFullYearIfYearHasNoAuto = false)
        {
            ...
            {
                var scopeDays = EnumerateDays(scope.Start!.Value, scope.End!.Value);
                using var cache = _eventService.BeginEventReadCache(_employeeId, scopeDays);
        ...
        private static List<DateTime> EnumerateDays(DateTime start, DateTime end)
        {
            var s = start.Date;
            var e = end.Date;
        ...
        private List<ChangedRange> BuildScopesForImport(ChangedRange changedRange)
        {
            ...
            {
                var chunkStart = year == src.Start.Value.Year
                    ? src.Start.Value
                    : new DateTime(year, 1, 1);

                var chunkEnd = year == src.End.Value.Year
                    ? src.End.Value
                    : new DateTime(year, 12, 31);

                bool hasAutoInYear = _eventService.HasAnyAutoGeneratedEventsInYear(year, _employeeId);

                if (hasAutoInYear)
                {
                    result.Add(new ChangedRange(chunkStart, chunkEnd));
                }
                else
                {
                    result.Add(new ChangedRange(
                        new DateTime(year, 1, 1),
                        new DateTime(year, 12, 31)));
                }
            }
