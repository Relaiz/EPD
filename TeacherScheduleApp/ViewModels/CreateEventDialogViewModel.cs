    ...
    public class CreateEventDialogViewModel : ViewModelBase
    {
        ...
        private async Task<Event?> CreateAsync()
        {
            ...
            if (SelectedEventTypePair.Key == EventType.Lunch)
            {
                ...
                var gross = resolvedForLunch.DepartureTime - resolvedForLunch.ArrivalTime;
                int lunchLimit =
                    gross >= TimeSpan.FromHours(12) ? 2 :
                    gross >= TimeSpan.FromHours(8) ? 1 : 0;

                if (lunchLimit == 0)
                {
                    await ShowValidationMessage.Handle("Pro den kratší než 8 hodin se oběd nevytváří.");
                    return null;
                }
                ...
                var existingLunchCount = _eventService.GetEventsForDay(_employeeId, day)
                    .Count(e =>
                        !e.IsDeleted &&
                ...
                if (existingLunchCount >= lunchLimit)
                {
                    await ShowValidationMessage.Handle(
                        lunchLimit == 1
