...
namespace TeacherScheduleApp.ViewModels
{
    ...
    .ToList();
            ...
            var manualForColumns = manual
                .Where(e => e.EventType == EventType.Work || e.EventType == EventType.Lunch)
                .ToList();

            var collisions = new HashSet<Event>();
            for (int i = 0; i < manualForColumns.Count; i++)
                for (int j = i + 1; j < manualForColumns.Count; j++)
                    if (manualForColumns[i].StartTime < manualForColumns[j].EndTime &&
                        manualForColumns[j].StartTime < manualForColumns[i].EndTime)
                    {
                        collisions.Add(manualForColumns[i]);
                        collisions.Add(manualForColumns[j]);
                    }
            ...
            foreach (var e in manualForColumns.Where(collisions.Contains))
            {
            ...
            {
                double sh = ev.AllDay ? ev.ArrivalTime.TimeOfDay.TotalHours : ev.StartHour;
                double eh = ev.AllDay ? ev.DepartureTime.TimeOfDay.TotalHours : ev.EndHour;
                if (eh <= sh) continue;

                bool isSpecial = ev.EventType != EventType.Work && ev.EventType != EventType.Lunch;
                ...
                var ctrl = new CalendarEventControl(ev)
                {
                    ...
                    StartHour = sh,
                    EndHour = eh,
                    OverlapCount = !isSpecial && pos.TryGetValue(ev, out var p) ? p.cnt : 1,
                    OverlapIndex = !isSpecial && pos.TryGetValue(ev, out p) ? p.idx : 0
                };
